...
from interchange import AssemblyData as AsmData, CadDocument as CadDoc, ComponentDefinition, ComponentDocument as ComponentDoc, ComponentInstance, ComponentKind, Diagnostic as DiagValue, Provenance, ProvenanceSpan, Severity, frozen_mapping as FrozenMapping
from convert.adapters.catia.Container import Cfv2Archive as CfvTwoArchive, Cfv2FormatError as CfvTwoFormatError, Cfv2Stream as CfvTwoStream
...
def DecodeProductA(Archive: Cfv2Archive) -> NativeProductB:
    ...
    Candidates.sort(key=lambda ItemValue: (ItemValue[0].name != 'Data', -len(ItemValue[1]), ItemValue[0].name.casefold()))
    ...
    Alternatives = tuple((NativeProductC(root_name=Value[1][1].value, stream_name=Value[0].name, stream_descriptor_offset=Value[0].descriptor_offset, table_offset=Value[1][0].offset, tokens=Value[1]) for Value in Candidates[1:] if len(Value[1]) >= 2))
    return NativeProductB(root_name=Tokens[1].value, stream_name=Stream.name, stream_descriptor_offset=Stream.descriptor_offset, table_offset=Tokens[0].offset, tokens=Tokens, occurrences=Occurrences, ambiguous_tokens=AmbiguousTokens, alternatives=Alternatives)

# this definition exists because focused behavior needs one stable owner
def PhysicalSpans(Archive: Cfv2Archive, Table: NativeProductTable, LogicalOffset: int, Length: int, RecordKind: str) -> tuple[ProvenanceSpan, ...]:
    Stream = next((ItemValue for ItemValue in Archive.outer.streams if ItemValue.descriptor_offset == Table.stream_descriptor_offset), None)
    if Stream is None:
    ...
    for Extent in Stream.extents:
        ...
        if OverlapStart >= OverlapEnd:
            continue
        PhysicalOffset = Archive.outer.physical_base + Extent.physical_offset + OverlapStart - ExtentStart
        OverlapLength = OverlapEnd - OverlapStart
        Spans.append(ProvenanceSpan(Table.stream_name, PhysicalOffset, OverlapLength, RecordKind))
...
def NativeProductE(Archive: Cfv2Archive, Label: str, Settings: ReadOptions, Reader: ComponentReader) -> tuple[AsmData, tuple[DiagValue, ...]]:
    ...
    Definitions: list[ComponentDefinition] = [ComponentDefinition(RootId, Table.root_name, ComponentKind.ASSEMBLY, source_path=str(RootPath) if RootPath is not None else Label, source_format_id=KFormatId, source_sha256=Hashlib.sha256(Archive.data).hexdigest() if Archive.data else '', provenance=Provenance(KFormatId, 'ASMPRODUCT', spans=PhysicalSpans(Archive, Table, Table.tokens[1].offset, Table.tokens[1].length, 'product-name')), attributes=FrozenMapping({'native_structure': 'ASMPRODUCT', 'native_string_table_logical_offset': Table.table_offset, 'native_string_table_physical_offset': PhysicalSpans(Archive, Table, Table.table_offset, 1, 'string-table-prefix')[0].offset}))]
    ...
    for DefinitionName in dict.fromkeys((ItemValue.definition_name for ItemValue in Table.occurrences)):
        ...
        KindValue = ComponentKind.ASSEMBLY if RefValue is not None and RefValue.document_type == ProductDocType else ComponentKind.PART if RefValue is not None and RefValue.document_type == PartDocType else ComponentKind.REFERENCE
        Definitions.append(ComponentDefinition(DefinitionId, DefinitionName, KindValue, document_id=DocId, body_ids=tuple((BodyValue.id for BodyValue in DocValue.bodies)) if DocValue is not None and KindValue == ComponentKind.PART else (), source_path=str(RefValue.path) if RefValue is not None else '', source_format_id=KFormatId if RefValue is not None else '', source_sha256=RefValue.sha256 if RefValue is not None else '', provenance=Provenance(KFormatId, DefinitionName, spans=PhysicalSpans(Archive, Table, FirstOccurrences[DefinitionName].definition_offset, FirstOccurrences[DefinitionName].definition_length, 'component-definition')), attributes=FrozenMapping({'native_reference_name': DefinitionName, 'source_resolved': RefValue is not None, 'source_ambiguous': len(RefCandidates.get(DefinitionName, ())) > 1, 'native_reference_candidates': tuple(({'path': str(Choice.path), 'document_type': Choice.document_type, 'sha256': Choice.sha256} for Choice in RefCandidates.get(DefinitionName, ())))})))
    Instances = tuple((ComponentInstance(f'catia:instance:{Order + 1}', ItemValue.instance_name, DefinitionIds[ItemValue.definition_name], RootId, order=Order, reference_number=ItemValue.reference_number, provenance=Provenance(KFormatId, ItemValue.instance_name, spans=PhysicalSpans(Archive, Table, ItemValue.instance_offset, ItemValue.instance_length, 'component-instance')), attributes=FrozenMapping({'native_definition_name': ItemValue.definition_name, 'native_string_logical_offset': ItemValue.instance_offset, 'native_string_physical_offset': PhysicalSpans(Archive, Table, ItemValue.instance_offset, ItemValue.instance_length, 'component-instance')[0].offset, 'transform_resolved': False, 'transform_source': 'exact_native_payload'})) for Order, ItemValue in enumerate(Table.occurrences)))
    Missing = tuple((NameValue for NameValue in DefinitionIds if NameValue not in Selected))
    Diagnostics: list[DiagValue] = [*SearchDiagnostics, *RefDiagnostics, *DocDiagnostics]
    if Table.alternatives:
        Diagnostics.append(DiagValue('catia.product.root_ambiguous', 'Multiple CATIA product tables were retained; the deterministic Data-first table supplies normalized assembly semantics.', Severity.WARNING, attributes=FrozenMapping({'selected': TableChoice(Table), 'alternatives': tuple((TableChoice(Choice) for Choice in Table.alternatives))})))
    if Table.ambiguous_tokens:
        Diagnostics.append(DiagValue('catia.product.native_tokens_retained', 'CATIA product tokens without verified occurrence roles remain available as ordered native records.', Severity.INFO, attributes=FrozenMapping({'tokens': tuple((TokenRecord(Token) for Token in Table.ambiguous_tokens))})))
    if Missing:
        Diagnostics.append(DiagValue('catia.product.component_sources_missing', f'{len(Missing)} CATProduct component sources could not be resolved by internal product name.', Severity.INFO, attributes=FrozenMapping({'definition_names': Missing})))
    if Instances:
        Diagnostics.append(DiagValue('catia.product.transforms_unresolved', 'CATProduct occurrence order and names are decoded; proprietary position records remain byte-exact in the native payload and unresolved transforms retain the identity default.', Severity.WARNING, attributes=FrozenMapping({'instance_count': len(Instances), 'resolved_count': 0, 'unresolved_default': 'identity'})))
    Diagnostics.append(DiagValue('catia.product.constraints_unresolved', 'CATProduct connector and constraint records remain byte-exact in the native payload; no semantic mates are asserted without a verified decoder.', Severity.INFO))
    AsmValue = AsmData(root_definition_id=RootId, definitions=tuple(Definitions), instances=Instances, documents=Documents, attributes=FrozenMapping({'native_structure': 'ASMPRODUCT', 'native_stream': Table.stream_name, 'native_string_table_logical_offset': Table.table_offset, 'native_string_table_physical_offset': PhysicalSpans(Archive, Table, Table.table_offset, 1, 'string-table-prefix')[0].offset, 'native_string_count': len(Table.tokens), 'native_instance_count': len(Instances), 'native_definition_count': len(DefinitionIds), 'resolved_definition_count': len(Selected), 'linked_document_count': len(Documents), 'linked_sketch_count': sum((len(ItemValue.document.sketches) for ItemValue in Documents)), 'linked_feature_count': sum((len(ItemValue.document.feature_timeline) for ItemValue in Documents)), 'transform_status': 'native-only', 'constraint_status': 'native-only', 'native_table_candidates': (TableChoice(Table), *(TableChoice(Choice) for Choice in Table.alternatives)), 'native_unresolved_tokens': tuple((TokenRecord(Token) for Token in Table.ambiguous_tokens)), 'native_reference_candidates': tuple(({'definition_name': NameValue, 'candidates': tuple(({'path': str(Choice.path), 'document_type': Choice.document_type, 'sha256': Choice.sha256} for Choice in Candidates))} for NameValue, Candidates in RefCandidates.items() if Candidates))}))
    return (AsmValue, tuple(Diagnostics))

# this definition exists because focused behavior needs one stable owner
def TokenRecord(Token: NativeProductToken) -> dict[str, object]:
    return {'value': Token.value, 'offset': Token.offset, 'length': Token.length, 'encoding': Token.encoding}

# this definition exists because focused behavior needs one stable owner
def TableChoice(Table: NativeProductTable | NativeProductTableCandidate) -> dict[str, object]:
    return {'root_name': Table.root_name, 'stream_name': Table.stream_name, 'stream_descriptor_offset': Table.stream_descriptor_offset, 'table_offset': Table.table_offset, 'tokens': tuple((TokenRecord(Token) for Token in Table.tokens))}
...
def DecodeProduct(RawValue: bytes) -> tuple[str, str]:
    ...
    if len(RawValue) >= 2 and len(RawValue) % 2 == 0:
        ...
        LittleZeroes = sum((RawValue[Index] == 0 for Index in range(1, len(RawValue), 2)))
    ...
    return (Decoded, 'utf-8') if Decoded is not None else (RawValue.decode('latin-1'), 'latin-1')
...
# this definition exists because focused behavior needs one stable owner
def Product(Tokens: tuple[NativeProductToken, ...]) -> tuple[tuple[NativeProduct, ...], tuple[NativeProductD, ...]]:
    Values = tuple((Token.value for Token in Tokens))
    try:
        Start = Values.index('_Reps') + 1
    except ValueError as exc:
        raise CfvTwoFormatError('CATIA ASMPRODUCT table has no _Reps boundary') from exc
    Result: list[NativeProduct] = []
    UsedValue: set[int] = set()
    Terminal = next((Index for Index in range(Start, len(Tokens)) if Tokens[Index].value == 'IsRoot'), len(Tokens))
    ...
    def Append(DefinitionIndex: int, InstanceIndex: int, RefValue: str) -> None:
        ...
        Result.append(NativeProduct(Definition.value, Instance.value, Definition.offset, Instance.offset, Definition.length, Instance.length, RefValue))
        UsedValue.update((DefinitionIndex, InstanceIndex))
    Marker = next((Index for Index in range(Start + 1, Terminal) if Tokens[Index].value == '_InstanceName'), None)
    ...
    if Marker is not None and Marker + 1 < Terminal:
        ...
        Shape = next((Index for Index in range(Marker + 2, Terminal) if Tokens[Index].value == 'Shape 1'), Marker + 1)
        PoolStart = Shape + 1
    ...
    for Index in range(PoolStart, Terminal):
        ...
        if Tokens[Index].value == '_InstanceName' and Pending is not None:
            ...
            if InstanceIndex < Terminal:
                Append(Pending, InstanceIndex, Identity[1] if (Identity := Numbered(Tokens[InstanceIndex].value)) is not None else '')
                if Identity is not None:
    ...
    if Terminal >= Start + 3:
        ...
        if Tokens[Ordinal].value.isdecimal() and Definition not in UsedValue and (Instance not in UsedValue):
            Append(Definition, Instance, Tokens[Ordinal].value)
    Ambiguous = tuple((Token for Index, Token in enumerate(Tokens) if Index >= Start and Index not in UsedValue))
...
# this definition exists because focused behavior needs one stable owner
def ComponentRef(Label: str, Settings: ReadOptions) -> tuple[dict[str, tuple[NativeProductA, ...]], tuple[DiagValue, ...]]:
    ...
    MaxTotalBytes = SearchLimit(Settings, 'component_search_max_total_bytes', KDefaultMaxTotalBytes)
    MaxDepth = SearchLimit(Settings, 'component_search_max_depth', KDefaultMaxDepth, AllowZero=True)
    ...
    for RootValue in Roots:
        ...
        while Pending and Limit is None:
            ...
            try:
                ...
                # this callback exists because local behavior needs one focused transformation
                Entries = tuple(sorted(Folder.iterdir(), key=lambda ItemValue: (ItemValue.name.casefold(), ItemValue.name)))
            except OSError as exc:
                Diagnostics.append(SearchDiag(Folder, 'unreadable_directory', str(exc)))
                continue
            for PathValue in Entries:
                ...
                try:
                    if PathValue.is_dir():
                        ...
                        if not UnderRoot(ResolvedFolder, RootValue):
                            Diagnostics.append(SearchDiag(PathValue, 'root_escape'))
                            continue
                        Pending.append((ResolvedFolder, Depth + 1))
                        continue
                    if not PathValue.is_file() or PathValue.suffix.casefold() not in DocTypeBySuffix:
                        continue
                    Resolved = PathValue.resolve(strict=True)
                    if not UnderRoot(Resolved, RootValue):
                        Diagnostics.append(SearchDiag(PathValue, 'root_escape'))
                        continue
                    SizeValue = Resolved.stat().st_size
                except OSError as exc:
                    Diagnostics.append(SearchDiag(PathValue, 'unreadable_candidate', str(exc)))
                    continue
                ...
                try:
                    ...
                    continue
                References[Table.root_name].append(NativeProductA(Table.root_name, Resolved, DocTypeBySuffix[Resolved.suffix.casefold()], Hashlib.sha256(DataValue).hexdigest()))
        if Limit is not None:
            break
    if Limit is not None:
        Diagnostics.append(DiagValue('catia.product.component_search_limit', f'CATIA component discovery stopped at the configured {Limit} limit.', Severity.WARNING, attributes=FrozenMapping({'limit': Limit, 'files': FileCount, 'total_bytes': TotalBytes, 'max_files': MaxFiles, 'max_total_bytes': MaxTotalBytes, 'max_depth': MaxDepth})))
...
def ComponentSearch(Label: str, Settings: ReadOptions) -> tuple[tuple[PathValue, ...], tuple[DiagValue, ...]]:
    ...
    for Choice in Candidates:
        ...
        try:
            ...
            Diagnostics.append(SearchDiag(Choice, 'unavailable_root', str(exc)))
            continue
...
# this definition exists because focused behavior needs one stable owner
def SearchLimit(Settings: ReadOptions, NameValue: str, Default: int, *, AllowZero: bool=False) -> int:
    ...
    try:
        ...
        raise ValueError(f'{NameValue} must be an integer') from exc
...
def Selected(Table: NativeProductTable, References: dict[str, tuple[NativeProductReference, ...]]) -> tuple[dict[str, NativeProductA], dict[str, tuple[NativeProductA, ...]], tuple[DiagValue, ...]]:
    ...
    Diagnostics: list[DiagValue] = []
    for NameValue in dict.fromkeys((ItemValue.definition_name for ItemValue in Table.occurrences)):
        Candidates = References.get(NameValue, ())
        if not Candidates:
            continue

        # this callback exists because local behavior needs one focused transformation
        Ordered = sorted(Candidates, key=lambda ItemValue: (str(ItemValue.path).casefold(), str(ItemValue.path)))
        Retained[NameValue] = tuple(Ordered)
        if len(Ordered) == 1:
            Selected[NameValue] = Ordered[0]
        else:
            Diagnostics.append(DiagValue('catia.product.component_source_ambiguous', f'Multiple CATIA documents declare product name {NameValue!r}; no source was selected without unique structural identity.', Severity.WARNING, attributes=FrozenMapping({'definition_name': NameValue, 'selected': '', 'candidates': tuple((str(ItemValue.path) for ItemValue in Ordered))})))
    return (Selected, Retained, tuple(Diagnostics))

# this definition exists because focused behavior needs one stable owner
def Component(Label: str, Table: NativeProductTable, References: dict[str, NativeProductReference], Settings: ReadOptions, Reader: ComponentReader) -> tuple[tuple[ComponentDoc, ...], dict[str, str], tuple[DiagValue, ...]]:
    Source = SourcePath(Label)
    ...
    for NameValue in Names:
        ...
        if str(RefValue.path).casefold() in Active:
            Diagnostics.append(DiagValue('catia.product.component_cycle', f'Recursive CATIA product reference was not expanded: {RefValue.path}', Severity.WARNING))
            continue
        ...
        Values['catia_path_stack'] = (*Stack, *((str(Source),) if Source is not None else ()))
        Options = ReadOptions(configuration=Settings.configuration, include_brep=Settings.include_brep, include_tessellation=Settings.include_tessellation, strict=Settings.strict, values=FrozenMapping(Values))
        try:
            DocValue = Reader(RefValue.path, Options)
        except (CfvTwoFormatError, OSError, TypeError, ValueError) as exc:
            Diagnostics.append(DiagValue('catia.product.component_decode_failed', f'CATIA component could not be decoded: {RefValue.path}: {exc}', Severity.WARNING, attributes=FrozenMapping({'definition_name': NameValue})))
            continue
        if DocValue.source.sha256.casefold() != RefValue.sha256.casefold():
            Diagnostics.append(DiagValue('catia.product.component_source_changed', f'CATIA component changed after discovery and was not linked: {RefValue.path}', Severity.WARNING, attributes=FrozenMapping({'definition_name': NameValue, 'indexed_sha256': RefValue.sha256, 'decoded_sha256': DocValue.source.sha256})))
            continue
