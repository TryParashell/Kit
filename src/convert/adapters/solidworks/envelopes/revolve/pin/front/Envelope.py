...
from convert.adapters.solidworks.programs.configuration.revolve.pin.front.Program import EncodeProgram as EncodePinFrontConfig
from convert.adapters.solidworks.container.Container import SldprtFormatError
from convert.adapters.solidworks.programs.resolved.revolve.pin.front.Program import EncodeProgram as EncodePinFrontFeatures
from convert.adapters.solidworks.envelopes.revolve.pin.default.Envelope import KPinPointsMm, PinEnvelope
...
def CalcPinFront(PointsMm: Sequence[tuple[float, float]]=KPinPointsMm) -> tuple[float, ...]:
    if len(PointsMm) < 3:
    ...
    SphereRadius = MathValue.sqrt((AxisMaximum - AxisMinimum) ** 2 * 0.25 + RadiusMetres ** 2 + RadiusMetres ** 2)
    return (0.0, AxisCentre, 0.0, RadiusMetres, AxisMaximum, RadiusMetres, -RadiusMetres, AxisMinimum, -RadiusMetres, SphereRadius)
...
def EncodeHeader() -> bytes:
    ...
    IdentityData = NativeMod._NativeIdentity(CreatedStamp, ModifiedStamp, BaselineStamp, HeaderStamp, NativeMod._SOLIDWORKS_CONFIGURATION_FLAGS, KHeaderLogRef)
    HeaderData = bytearray(NativeMod._header_payload(IdentityData, 'Default', (*NativeMod._HEADER_OBJECTS, (26, 'Sketch1', True), (31, 'Revolve1', False)), '', KHeaderUser, 32, {26: KHeaderStamps[0], 31: KHeaderStamps[1]}, CalcPinFront()))
