...
from convert.adapters.solidworks.programs.configuration.revolve.pin.rightangle.Program import EncodeProgram as EncodePinNineZeroConfig
from convert.adapters.solidworks.container.Container import SldprtFormatError
from convert.adapters.solidworks.programs.resolved.revolve.pin.rightangle.Program import EncodeProgram as EncodePinNineZeroFeatures
...
def CalcPinNineZero(PointsMm: Sequence[tuple[float, float]]=KPinPointsMm) -> tuple[float, ...]:
    if len(PointsMm) < 3:
    ...
    SphereRadius = MathValue.sqrt(RadiusCentre ** 2 * 2.0 + (AxisMaximum - AxisMinimum) ** 2 * 0.25)
    return (RadiusCentre, RadiusCentre, AxisCentre, RadiusMetres, RadiusMetres, AxisMaximum, 0.0, 0.0, AxisMinimum, SphereRadius)
...
def EncodeHeader() -> bytes:
    ...
    return NativeMod._header_payload(IdentityData, 'Default', (*NativeMod._HEADER_OBJECTS, (26, 'Sketch1', True), (31, 'Revolve1', False)), '', KHeaderUser, 32, {26: KHeaderStamps[0], 31: KHeaderStamps[1]}, CalcPinNineZero())
