...
from interchange import BooleanOperation as BoolOperation, ConstraintKind as RuleKind, ExtrusionEndCondition, FeatureKind, GeometryKind as GeomKind, MateEntityKind, MateKind, ValueKind
...
# this binding exists because shared behavior needs one stable value
KJointRefIndexByProp = {NameValue: Index for Index, NameValue in enumerate(KJointRefProperties)}
...
# this binding exists because shared behavior needs one stable value
KSubElemMateEntityKinds = (MateEntityKind.FACE, MateEntityKind.EDGE, MateEntityKind.VERTEX, MateEntityKind.AXIS, MateEntityKind.PLANE)

# this binding exists because shared behavior needs one stable value
KSubElemKindByPrefix = {KindValue.value.title(): KindValue for KindValue in KSubElemMateEntityKinds}

# this binding exists because shared behavior needs one stable value
KSupportPlaneTypeIds = frozenset({'App::Plane', 'Part::DatumPlane', 'PartDesign::Plane'})

# this binding exists because shared behavior needs one stable value
KQuantityPropUnits = {'Acceleration': 'mm/s^2', 'AmountOfSubstance': 'mol', 'Angle': 'deg', 'Area': 'mm^2', 'CompressiveStrength': 'kg/(mm*s^2)', 'CurrentDensity': 'A/mm^2', 'Density': 'kg/mm^3', 'DissipationRate': 'mm^2/s^3', 'Distance': 'mm', 'DynamicViscosity': 'kg/(mm*s)', 'ElectricalCapacitance': 'A^2*s^4/(kg*mm^2)', 'ElectricalConductance': 'A^2*s^3/(kg*mm^2)', 'ElectricalConductivity': 'A^2*s^3/(kg*mm^3)', 'ElectricalInductance': 'kg*mm^2/(A^2*s^2)', 'ElectricalResistance': 'kg*mm^2/(A^2*s^3)', 'ElectricCharge': 'A*s', 'ElectricCurrent': 'A', 'ElectricPotential': 'kg*mm^2/(A*s^3)', 'ElectromagneticPotential': 'kg*mm/(A*s^2)', 'Force': 'kg*mm/s^2', 'Frequency': '1/s', 'HeatFlux': 'kg/s^3', 'InverseArea': '1/mm^2', 'InverseLength': '1/mm', 'InverseVolume': '1/mm^3', 'KinematicViscosity': 'mm^2/s', 'Length': 'mm', 'LuminousIntensity': 'cd', 'MagneticFieldStrength': 'A/mm', 'MagneticFlux': 'kg*mm^2/(A*s^2)', 'MagneticFluxDensity': 'kg/(A*s^2)', 'Magnetization': 'A/mm', 'Mass': 'kg', 'Moment': 'kg*mm^2/s^2', 'Power': 'kg*mm^2/s^3', 'Pressure': 'kg/(mm*s^2)', 'ShearModulus': 'kg/(mm*s^2)', 'SpecificEnergy': 'mm^2/s^2', 'SpecificHeat': 'mm^2/(s^2*K)', 'Speed': 'mm/s', 'Stiffness': 'kg/s^2', 'StiffnessDensity': 'kg/(mm^2*s^2)', 'Stress': 'kg/(mm*s^2)', 'SurfaceChargeDensity': 'A*s/mm^2', 'Temperature': 'K', 'ThermalConductivity': 'kg*mm/(s^3*K)', 'ThermalExpansionCoefficient': '1/K', 'ThermalTransferCoefficient': 'kg/(s^3*K)', 'Time': 's', 'UltimateTensileStrength': 'kg/(mm*s^2)', 'VacuumPermittivity': 'A^2*s^4/(kg*mm^3)', 'Velocity': 'mm/s', 'Volume': 'mm^3', 'VolumeChargeDensity': 'A*s/mm^3', 'VolumeFlowRate': 'mm^3/s', 'VolumetricThermalExpansionCoefficient': '1/K', 'Work': 'kg*mm^2/s^2', 'YieldStrength': 'kg/(mm*s^2)', 'YoungsModulus': 'kg/(mm*s^2)'}
...
# this binding exists because shared behavior needs one stable value
KScalarPropKinds = {**{f'App::Property{NameValue}': (ValueKind.QUANTITY, UnitValue, 'Float') for NameValue, UnitValue in KQuantityPropUnits.items()}, **{Value.type_id: (Value.value_kind, Value.unit, Value.value_tag) for Value in KScalarPropTypes}}
...
# this binding exists because shared behavior needs one stable value
KBoolOperationTypeByKind = {Value.operation.value: Value for Value in KBoolOperationTypes}

# this binding exists because shared behavior needs one stable value
KCreateOperationNames = frozenset({'', BoolOperation.CREATE.value})

# this binding exists because shared behavior needs one stable value
KFeatureWriteTypeIds = MappingProxyType({FeatureKind.EXTRUSION: frozenset((Value.type_id for Value in KBoolOperationTypes)), FeatureKind.REVOLUTION: frozenset(), FeatureKind.SWEEP: frozenset(), FeatureKind.LOFT: frozenset(), FeatureKind.HOLE: frozenset(), FeatureKind.HELIX: frozenset(), FeatureKind.FILLET: frozenset({'Part::Fillet'}), FeatureKind.CHAMFER: frozenset(), FeatureKind.SHELL: frozenset(), FeatureKind.DRAFT: frozenset(), FeatureKind.PATTERN: frozenset(), FeatureKind.MIRROR: frozenset(), FeatureKind.SCALE: frozenset(), FeatureKind.OFFSET: frozenset(), FeatureKind.PRIMITIVE: frozenset(), FeatureKind.SURFACE: frozenset(), FeatureKind.REFINE: frozenset(), FeatureKind.REVERSE: frozenset(), FeatureKind.BOOLEAN: frozenset(), FeatureKind.IMPORTED: frozenset({'Part::Feature'}), FeatureKind.REFERENCE: frozenset(), FeatureKind.NATIVE: frozenset()})
if KFeatureWriteTypeIds.keys() != set(FeatureKind):
    raise RuntimeError('FreeCAD feature write types are not exhaustive')

# this binding exists because shared behavior needs one stable value
KFeatureWriteKinds = frozenset((KindValue for KindValue, TypeIds in KFeatureWriteTypeIds.items() if TypeIds))
...
# this binding exists because shared behavior needs one stable value
KExtrusionTypes = (ExtrusionType(0, ExtrusionEndCondition.BLIND), ExtrusionType(1, ExtrusionEndCondition.UP_TO_LAST, ExtrusionEndCondition.THROUGH_ALL), ExtrusionType(2, ExtrusionEndCondition.UP_TO_FIRST), ExtrusionType(3, ExtrusionEndCondition.UP_TO_FACE), ExtrusionType(4, ExtrusionEndCondition.TWO_LENGTHS), ExtrusionType(5, ExtrusionEndCondition.UP_TO_SHAPE))
...
# this binding exists because shared behavior needs one stable value
KPrimitiveFeatureTypeIds = frozenset((f'{Family.namespace}::{Prefix}{Shape}' for Family in KPrimitiveFeatureFamilies for Prefix in Family.prefixes for Shape in Family.shapes))

# this binding exists because shared behavior needs one stable value
KPartObjectTypeIds = frozenset((*KFeatureKindByTypeId, *KPrimitiveFeatureTypeIds, *KSupportPlaneTypeIds, *KBodyContainerTypeIds))
...
# this binding exists because shared behavior needs one stable value
KRulePoints = (RulePoint(1, 'start', ('startpoint',)), RulePoint(2, 'end', ('endpoint',)), RulePoint(3, 'center', ('centre', 'midpoint')))

# this binding exists because shared behavior needs one stable value
KRulePointByIndex = {Value.index: Value.name for Value in KRulePoints}

# this binding exists because shared behavior needs one stable value
KRulePointIndexByName = {NameValue: Value.index for Value in KRulePoints for NameValue in (Value.name, *Value.aliases)}

# this binding exists because shared behavior needs one stable value
KMidpointRefPointNames = frozenset(('', 'mid', *(NameValue for Value in KRulePoints if Value.index == 3 for NameValue in (Value.name, *Value.aliases))))
...
# this binding exists because shared behavior needs one stable value
KRuleTypes = (RuleType(1, RuleKind.COINCIDENT, write_kinds=(RuleKind.CONCENTRIC,)), RuleType(2, RuleKind.HORIZONTAL), RuleType(3, RuleKind.VERTICAL), RuleType(4, RuleKind.PARALLEL), RuleType(5, RuleKind.TANGENT), RuleType(6, RuleKind.DISTANCE, ValueKind.LENGTH, 'mm'), RuleType(7, RuleKind.DISTANCE_X, ValueKind.LENGTH, 'mm'), RuleType(8, RuleKind.DISTANCE_Y, ValueKind.LENGTH, 'mm'), RuleType(9, RuleKind.ANGLE, ValueKind.ANGLE, 'rad'), RuleType(10, RuleKind.PERPENDICULAR), RuleType(11, RuleKind.RADIUS, ValueKind.LENGTH, 'mm'), RuleType(12, RuleKind.EQUAL), RuleType(13, RuleKind.POINT_ON_OBJECT), RuleType(14, RuleKind.SYMMETRIC), RuleType(15, RuleKind.INTERNAL_ALIGNMENT), RuleType(16, RuleKind.SNELLS_LAW, ValueKind.NUMBER), RuleType(17, RuleKind.BLOCK, write_kinds=(RuleKind.FIXED,)), RuleType(18, RuleKind.DIAMETER, ValueKind.LENGTH, 'mm'), RuleType(19, RuleKind.WEIGHT, ValueKind.NUMBER), RuleType(20, RuleKind.GROUP), RuleType(21, RuleKind.TEXT))

# this binding exists because shared behavior needs one stable value
KRuleKindByCode = {Value.code: Value.kind for Value in KRuleTypes}

# this binding exists because shared behavior needs one stable value
KRuleValueKindByCode = {Value.code: (Value.value_kind, Value.unit) for Value in KRuleTypes if Value.value_kind is not None}

# this binding exists because shared behavior needs one stable value
KRuleCodeByKind = {KindValue.value: Value.code for Value in KRuleTypes for KindValue in (Value.kind, *Value.write_kinds)}

# this binding exists because shared behavior needs one stable value
KRuleWriteCodes = MappingProxyType({RuleKind.COINCIDENT: frozenset({1}), RuleKind.HORIZONTAL: frozenset({2}), RuleKind.VERTICAL: frozenset({3}), RuleKind.PARALLEL: frozenset({4}), RuleKind.PERPENDICULAR: frozenset({10}), RuleKind.TANGENT: frozenset({5}), RuleKind.EQUAL: frozenset({12}), RuleKind.CONCENTRIC: frozenset({1}), RuleKind.POINT_ON_OBJECT: frozenset({13}), RuleKind.SYMMETRIC: frozenset({14}), RuleKind.MIDPOINT: frozenset({14}), RuleKind.DISTANCE: frozenset({6}), RuleKind.DISTANCE_X: frozenset({7}), RuleKind.DISTANCE_Y: frozenset({8}), RuleKind.ANGLE: frozenset({9}), RuleKind.RADIUS: frozenset({11}), RuleKind.DIAMETER: frozenset({18}), RuleKind.FIXED: frozenset({17}), RuleKind.INTERNAL_ALIGNMENT: frozenset({15}), RuleKind.SNELLS_LAW: frozenset({16}), RuleKind.BLOCK: frozenset({17}), RuleKind.WEIGHT: frozenset({19}), RuleKind.GROUP: frozenset({20}), RuleKind.TEXT: frozenset({21}), RuleKind.NATIVE: frozenset()})
...
# this binding exists because shared behavior needs one stable value
KRuleWriteKinds = frozenset((KindValue for KindValue, Codes in KRuleWriteCodes.items() if Codes))
...
# this binding exists because shared behavior needs one stable value
KFixedRuleKinds = frozenset((KindValue for KindValue, CodeValue in KRuleCodeByKind.items() if CodeValue == KRuleCodeByKind[RuleKind.BLOCK.value]))
...
# this binding exists because shared behavior needs one stable value
KGeomTypes = (GeomType('Part::GeomPoint', GeomKind.POINT, 'PointGeometry', True), GeomType('Part::GeomLine', GeomKind.LINE), GeomType('Part::GeomLineSegment', GeomKind.LINE, 'LineGeometry', True), GeomType('Part::GeomCircle', GeomKind.CIRCLE, 'CircleGeometry', True), GeomType('Part::GeomArcOfCircle', GeomKind.ARC, 'ArcGeometry', True), GeomType('Part::GeomEllipse', GeomKind.ELLIPSE, 'EllipseGeometry', True), GeomType('Part::GeomArcOfEllipse', GeomKind.ARC_ELLIPSE, 'ArcEllipseGeometry', True), GeomType('Part::GeomHyperbola', GeomKind.HYPERBOLA, 'HyperbolaGeometry'), GeomType('Part::GeomArcOfHyperbola', GeomKind.ARC_HYPERBOLA, 'ArcHyperbolaGeometry', True), GeomType('Part::GeomParabola', GeomKind.PARABOLA, 'ParabolaGeometry'), GeomType('Part::GeomArcOfParabola', GeomKind.ARC_PARABOLA, 'ArcParabolaGeometry', True), GeomType('Part::GeomBezierCurve', GeomKind.BEZIER, 'SplineGeometry', True), GeomType('Part::GeomBSplineCurve', GeomKind.SPLINE, 'SplineGeometry', True), GeomType('Part::GeomOffsetCurve', GeomKind.OFFSET), GeomType('Part::GeomTrimmedCurve', GeomKind.TRIMMED))

# this binding exists because shared behavior needs one stable value
KGeomKindByTypeId = {Value.type_id: Value.kind for Value in KGeomTypes}

# this binding exists because shared behavior needs one stable value
KGeomTypeIdsByKind = {KindValue.value: frozenset((Value.type_id for Value in KGeomTypes if Value.kind == KindValue)) for KindValue in GeomKind if KindValue != GeomKind.NATIVE}

# this binding exists because shared behavior needs one stable value
KNeutralGeomTypeByKind = {Value.kind.value: Value.neutral_type for Value in KGeomTypes if Value.neutral_default}

# this binding exists because shared behavior needs one stable value
KNeutralGeomTypeIdByKind = {Value.kind.value: Value.type_id for Value in KGeomTypes if Value.neutral_default}

# this binding exists because shared behavior needs one stable value
KGeomWriteTypeIds = MappingProxyType({GeomKind.POINT: frozenset({'Part::GeomPoint'}), GeomKind.LINE: frozenset({'Part::GeomLineSegment'}), GeomKind.CIRCLE: frozenset({'Part::GeomCircle'}), GeomKind.ARC: frozenset({'Part::GeomArcOfCircle'}), GeomKind.ELLIPSE: frozenset({'Part::GeomEllipse'}), GeomKind.ARC_ELLIPSE: frozenset({'Part::GeomArcOfEllipse'}), GeomKind.HYPERBOLA: frozenset(), GeomKind.ARC_HYPERBOLA: frozenset({'Part::GeomArcOfHyperbola'}), GeomKind.PARABOLA: frozenset(), GeomKind.ARC_PARABOLA: frozenset({'Part::GeomArcOfParabola'}), GeomKind.BEZIER: frozenset({'Part::GeomBezierCurve'}), GeomKind.SPLINE: frozenset({'Part::GeomBSplineCurve'}), GeomKind.OFFSET: frozenset(), GeomKind.TRIMMED: frozenset(), GeomKind.NATIVE: frozenset()})
if KGeomWriteTypeIds.keys() != set(GeomKind):
    raise RuntimeError('FreeCAD geometry write types are not exhaustive')

# this binding exists because shared behavior needs one stable value
KGeomWriteKinds = frozenset((KindValue for KindValue, TypeIds in KGeomWriteTypeIds.items() if TypeIds))
...
# this binding exists because shared behavior needs one stable value
KSplineGeomTypeIds = frozenset((Value.type_id for Value in KGeomTypes if Value.kind.value in KSplineGeomKinds))
...
# this binding exists because shared behavior needs one stable value
KJointTypeDefinitions = (JointType('Fixed', MateKind.LOCK, write_aliases=('fixed',)), JointType('Revolute', MateKind.HINGE, write_aliases=('revolute',)), JointType('Cylindrical', MateKind.CONCENTRIC, write_aliases=('cylindrical',)), JointType('Slider', MateKind.SLIDER), JointType('Ball', MateKind.BALL), JointType('Distance', MateKind.DISTANCE, uses_distance=True), JointType('Parallel', MateKind.PARALLEL), JointType('Perpendicular', MateKind.PERPENDICULAR), JointType('Angle', MateKind.ANGLE), JointType('RackPinion', MateKind.RACK_PINION, write_aliases=('rackpinion',), uses_distance=True), JointType('Screw', MateKind.SCREW, uses_distance=True), JointType('Gears', MateKind.GEAR, write_aliases=('gears',), uses_distance=True, uses_second_distance=True), JointType('Belt', MateKind.BELT, uses_distance=True, uses_second_distance=True))

# this binding exists because shared behavior needs one stable value
KMateKindByJointType = {Value.name: Value.kind for Value in KJointTypeDefinitions}

# this binding exists because shared behavior needs one stable value
KJointTypeByMateKind = {KeyValue: Value.name for Value in KJointTypeDefinitions for KeyValue in (Value.kind.value, *(KindValue.value for KindValue in Value.write_kinds), *Value.write_aliases)}

# this binding exists because shared behavior needs one stable value
KMateWriteTypes = MappingProxyType({MateKind.COINCIDENT: frozenset(), MateKind.CONCENTRIC: frozenset({'Cylindrical'}), MateKind.PARALLEL: frozenset({'Parallel'}), MateKind.PERPENDICULAR: frozenset({'Perpendicular'}), MateKind.TANGENT: frozenset(), MateKind.DISTANCE: frozenset({'Distance'}), MateKind.ANGLE: frozenset({'Angle'}), MateKind.LOCK: frozenset({'Fixed'}), MateKind.GEAR: frozenset({'Gears'}), MateKind.RACK_PINION: frozenset({'RackPinion'}), MateKind.SCREW: frozenset({'Screw'}), MateKind.COORDINATE: frozenset(), MateKind.SLIDER: frozenset({'Slider'}), MateKind.UNIVERSAL_JOINT: frozenset(), MateKind.CAM: frozenset(), MateKind.SLOT: frozenset(), MateKind.BALL: frozenset({'Ball'}), MateKind.WIDTH: frozenset(), MateKind.SYMMETRIC: frozenset(), MateKind.LINEAR_COUPLER: frozenset(), MateKind.BELT: frozenset({'Belt'}), MateKind.PATH: frozenset(), MateKind.MAGNETIC: frozenset(), MateKind.HINGE: frozenset({'Revolute'}), MateKind.PROFILE_CENTER: frozenset(), MateKind.NATIVE: frozenset()})
if KMateWriteTypes.keys() != set(MateKind):
    raise RuntimeError('FreeCAD mate write types are not exhaustive')

# this binding exists because shared behavior needs one stable value
KMateWriteKinds = frozenset((KindValue for KindValue, Types in KMateWriteTypes.items() if Types))
...
# this binding exists because shared behavior needs one stable value
KJointTypesUsingDistance = frozenset((Value.name for Value in KJointTypeDefinitions if Value.uses_distance))

# this binding exists because shared behavior needs one stable value
KJointTypesUsingSecond = frozenset((Value.name for Value in KJointTypeDefinitions if Value.uses_second_distance))

# this binding exists because shared behavior needs one stable value
KMateKindsUsingDistance = frozenset((Value.kind for Value in KJointTypeDefinitions if Value.uses_distance))

# this binding exists because shared behavior needs one stable value
KMateKindsUsingSecond = frozenset((Value.kind for Value in KJointTypeDefinitions if Value.uses_second_distance))
