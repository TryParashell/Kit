...
of the *same* part, so they double as controls. `CONTROL2PAD_A/B` are two runs of the same
...
Two effects change the length between *identically parameterised* runs:
...
1. The transient session document name (`Part2`, `Part70`, …) embedded twice as UTF-16 in
   ...
   become **16593** bytes.
2. **New:** a zlib-compressed Parasolid transmit blob (§3) whose *compressed* size varies
...
* Report 1's claim that "you never need to author Parasolid" survives, but only just: this blob is
  inside the *feature* stream, not the geometry cache, so it cannot be deleted like
...
bbox caches, now driven by the *total* height `10+d` — confirming they are derived from the final
...
*after* the name identify the node:
...
and +33 sit at fixed negative offsets — but at *different* offsets for the first feature and for
later ones:
...
for a plane-supported rectangular second feature but 6670 for a face-supported one. What *is*
...
2. **Only the first feature carries `Type="Boss-Extrude"`.** Features 2 and 3 have no `Type`
   ...
   DissectableRoot="true"` instead. So in KeyWords, boss versus cut is distinguishable *only* by
...
## 10. What can be donor-patched, and what cannot

**Donor-patching is sufficient** — proven by §9 — for any part that reuses a donor with the same
*feature topology*, i.e. the same ordered sequence of operations, profile types, support types and
...
| `CUTMID` (MidPlane cut) | **donor-patch**, plus one byte at scalar−715 if you need to switch a blind donor to MidPlane. Not round-trip-tested as a *switch*; the read-back is verified on 11 files. |
...
| `CUTTHROUGH` (ThroughAll) | **needs a donor of its own.** Switching blind ↔ ThroughAll adds/removes a 1723-byte dimension object and a whole class record. Patchable only from a ThroughAll donor, where there is nothing left to patch except the sketch. |
| changing feature **count** (2 → 3) or **operation** (boss ↔ cut) | **needs real serialization.** Feature count changes the class-declaration layout, the object-index numbering, the `moICE_c`/`moCompFace_c` chain and every id in `moCompFeature_c`, and adds a `<Extrusion>`/`<Sketch>` pair to KeyWords. Boss ↔ cut is one `u32` in the stream but also changes the feature *name* string (`Cut-Extrude1` ↔ `Boss-Extrude2`), which is variable-length and appears in both the resolved stream and KeyWords, and changes `moEndFace3IntSurfIdRep_c` presence. I would not attempt either by patching. |
...
Practical consequence: a converter can cover a useful envelope with a small donor library keyed by
`(ordered list of (operation, profile type, support type, end condition))`, with all numerics
patched. It does not need a serializer until it needs to vary that key.
