# `resolved.py` findings: feature flag words and the arc/circle record

Scope: `src/convert/adapters/solidworks/resolved.py` and `tests/convert/test_solidworks_resolved.py` only.
...
- `uv run python -m pytest tests/convert/test_solidworks_resolved.py -q` → 55 passed (was 43).
- `uv run python -m pytest tests/convert/test_solidworks.py tests/convert/test_solidworks_writer.py
  tests/convert/test_solidworks_adapter.py -q` → 2 failed, 132 passed. Both failures are the
