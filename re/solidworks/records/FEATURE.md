...
Confidence words as in `SERIALIZE.md`: **confirmed** = decompiled *and* the arithmetic reproduces
...
* **On load** the reader does `*(uint *)(this + 0x28) |= 0x40000000` unconditionally. Bit 30 is
  ...
  word has `0x40000000` set. (The legacy `moNode_c::Serialize` instead ORs bit 30 back *from the
  pre-read in-memory value*, `|= uVar2 & 0x40000000` — same effect for a fresh load.)
...
`moFeature_c + 0x290` is read *before* any version gate and the reader immediately branches on it
...
* Everything after `moFeature_c + 0x290` is still donor territory. `ANSWERS.md` Q1's conclusion
  stands and is now better explained: flipping the flags word alone leaves the derived
  face-identity records (`moEndFaceSurfIdRep_c`, `moFromSktEntSurfIdRep_c`, the `moFR_c` id words)
  describing the wrong faces, and those live in `FUN_4bb886c0`'s and `moBodyFeature_c`'s regions.
