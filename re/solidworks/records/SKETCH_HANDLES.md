...
`re/tooling/ghidra/verify_class_layouts.py` replays the declared layout in
...
`re/tooling/ghidra/solve_runs.py` reports.
...
must be discarded, not modelled. `class_layouts_decompiled.json` declares
...
next traced object is 46 bytes, so no instance pins the total. `class_layouts_decompiled.json`
...
All three inherit `sgEntHandle::Serialize` unchanged — `serialize_map.json` gives the same slot-5
...
`solve_runs.py` derives `sgPointHandle@leaf = 2` with 114 witnesses. **That value is wrong.** It
...
`solve_runs.py` reports are the same trace-attribution artefact as §0. One instance,
...
`sgLLDist`'s. `verify_class_layouts.py` reports exactly one mismatch across all nine traces and it
...
(`re/solidworks/records/FEATURE_BODY.md` §3), whose count is a `u16` at the start of the list
object's own body.
