...
Census tooling: `Kit/.rescratch/census.py` -> `Kit/.rescratch/census/census.txt`, `per_file.json`.
...
`resolved.py` had `BOSS_FLAGS = 0x40000140` / `CUT_FLAGS = 0x400201CA`. Across all 63 corpus files the observed
...
`uv run python -m pytest tests/convert/test_solidworks_adapter.py::test_entire_local_solidworks_corpus_decodes`
...
`uv run python -m pytest tests -q --ignore=tests/convert/test_swaps.py --ignore=tests/convert/test_random_assembly.py`
=> `8 failed, 808 passed, 8 skipped`. The 8:

- `test_api.py::test_readme_describes_default_reversible_swaps_and_strict_mode`
- `test_catia_assembly.py::test_catproduct_to_embedded_fcstd_structural_roundtrip`
- `test_freecad_assembly.py::test_fcstd_assembly_has_component_links_placements_and_mates`
- `test_freecad_brep.py::test_supplied_solidworks_breps_pass_only_the_proven_native_gate`
- `test_solidworks.py::test_protocol_literals_have_one_source_definition`
- `test_solidworks_adapter.py::test_entire_local_solidworks_corpus_decodes`  <- new, caused by this corpus
- `test_solidworks_assembly.py::test_assembly_capabilities_reflect_the_decoded_document`
- `test_solidworks_assembly.py::test_mate_list_discovery_uses_structure_when_the_stream_is_renamed`
