...
class Field:
    ...
    def ReadData(SelfRef, ByteBlob: bytes) -> int:
        return int.from_bytes(ByteBlob[SelfRef.Offset:SelfRef.Offset + SelfRef.WidthInfo], 'little')


    # needed to keep reverse engineering responsibilities isolated and maintainable
    def Write(SelfRef, ByteBlob: bytes, ValueInfo: int) -> bytes:
        Output = bytearray(ByteBlob)
        Output[SelfRef.Offset:SelfRef.Offset + SelfRef.WidthInfo] = ValueInfo.to_bytes(SelfRef.WidthInfo, 'little')
        return bytes(Output)
...
# needed to keep reverse engineering responsibilities isolated and maintainable
KGroups = {'header': (KFields[0], KFields[1]), 'cmgr': (KFields[2],), 'all': KFields, 'none': ()}
...
# needed to keep reverse engineering responsibilities isolated and maintainable
def PatchedStreams(DonorInfo: Streamlib.Donor, FeatInfoInfo: int, Group: str) -> dict[str, bytes]:
    Result: dict[str, bytes] = {}
    for FieldInfo in KGroups[Group]:
        Source = Result.get(FieldInfo.stream, DonorInfo.streams[FieldInfo.stream])
        Result[FieldInfo.stream] = FieldInfo.write(Source, FieldInfo.expected(FeatInfoInfo))
    return Result
...
def MainRun() -> int:
    for ItemData in System.argv[1:]:
        ...
        for FieldInfo in KFields:
            ...
            RowDataInfo.append(f"{FieldInfo.stream.split('/')[-1]}[{FieldInfo.offset}]={FieldInfo.read(ByteBlob)}/{FieldInfo.expected(FeatInfoInfo)}")
