...
# needed to keep reverse engineering responsibilities isolated and maintainable
KStreamSpecs = (('Contents/CMgr', 'cmgr'), ('Contents/Config-0', 'config'), ('Contents/Config-0-ResolvedFeatures', 'resolved'), ('Contents/Definition', 'definition'), ('Contents/Config-0-ModelHeader', 'header'))

# needed to keep reverse engineering responsibilities isolated and maintainable
KDirectInfo = {'Contents/CMgr': ((0, 'H', 'configuration manager archive prefix'), (6, '5I', 'configuration manager version preamble'), (785, 'H', 'configuration manager inline object tag'), (1872, 'H', 'configuration manager inline object tag')), 'Contents/Config-0': ((0, 'H', 'assembly configuration archive prefix'), (6, '3I', 'assembly configuration version preamble'), (2606, '2d', 'moTransRefPlaneData_c first inline transform row'), (2626, '2d', 'moTransRefPlaneData_c second inline transform row'), (2830, 'I', 'moTransRefPlaneData_c inline flag'), (17610, '3i', 'moLengthUserUnits_c detail scalar triplet'), (17662, '3i', 'moRelMgr_c detail scalar triplet'), (24076, 'IHH8B', 'moSketchBlockMgr_c persistent identifier')), 'Contents/Config-0-ResolvedFeatures': ((0, 'I', 'resolved feature continuation base'),), 'Contents/Definition': ((0, 'I', 'definition document flags'), (28, 'd', 'definition document class identifier tail'), (3621, 'H', 'definition journal inline state'), (3654, 'I', 'definition journal inline state')), 'Contents/Config-0-ModelHeader': ((0, 'H', 'model header archive prefix'), (6, '5H', 'model header string-array preamble'), (41, '7H', 'model header user and log preamble'), (57, '3H', 'model header log-list preamble'))}

# needed to keep reverse engineering responsibilities isolated and maintainable
KStatusField = ('Contents/Config-0', 3477, ('Description', 'High Priority', 'Low Priority', 'Complete', 'Reminder'), 'moRelMgr_c status-name table')

# needed to keep reverse engineering responsibilities isolated and maintainable
KRefLengths = {'Contents/CMgr': 1914, 'Contents/Config-0': 24477, 'Contents/Config-0-ResolvedFeatures': 5442, 'Contents/Definition': 3894, 'Contents/Config-0-ModelHeader': 2471}
...
def TraceRows(TraceText: str) -> tuple[tuple[str, int, str, int], ...]:
    ...
    for SourceLine in TraceText.splitlines():
        ...
        ResultRows.append((PartsList[1], int(PartsList[2], 16), OwnerText, int(PartsList[-1], 16)))
...
# needed to keep reverse engineering responsibilities isolated and maintainable
def AddStrings(Operations: list[tuple[int, int, str, str, AnyInfo]], Covered: set[int], StreamData: bytes, RowsList: tuple[tuple[str, int, str, int], ...]) -> None:
    StringStarts = {StartPos for TypeName, StartPos, OwnerText, StreamSpan in RowsList if TypeName == 'uchar' and 'su_CArchive::WriteString+0x53' in OwnerText and (StreamData[StartPos:StartPos + 3] == b'\xff\xfe\xff')}
...
# needed to keep reverse engineering responsibilities isolated and maintainable
def FieldPosition(StreamName: str, StartPos: int, OwnerText: str, StreamShift: int, Segments: tuple[dict[str, AnyInfo], ...]) -> int:
    if StreamName == 'Contents/Config-0' and OwnerText.startswith('moTransRefPlaneData_c') and OwnerText.endswith('inline transform row'):
        ClassStart = next((int(Segment['offset']) for Segment in Segments if Segment['kind'] == 'definition' and Segment['class_name'] == 'moTransRefPlaneData_c'))
...
# needed to keep reverse engineering responsibilities isolated and maintainable
def AddDirectFields(StreamName: str, Operations: list[tuple[int, int, str, str, AnyInfo]], Covered: set[int], StreamData: bytes, Segments: tuple[dict[str, AnyInfo], ...]) -> None:
    ...
    AddOpMut(Operations, Covered, StartPos, FieldWidth, OwnerText, 'stringlist', TextValues)


# needed to keep reverse engineering responsibilities isolated and maintainable
def AddPrimitives(Operations: list[tuple[int, int, str, str, AnyInfo]], Covered: set[int], StreamData: bytes, RowsList: tuple[tuple[str, int, str, int], ...]) -> None:
...
# needed to keep reverse engineering responsibilities isolated and maintainable
def BuildProgram(StreamName: str, StreamData: bytes, Segments: tuple[dict[str, AnyInfo], ...], TraceData: tuple[tuple[str, int, str, int], ...]) -> list[tuple[int, int, str, str, AnyInfo]]:
...
def RenderSource(Programs: dict[str, list[tuple[int, int, str, str, AnyInfo]]]) -> str:
    OwnerNames = tuple(sorted({Operation[2] for Operations in Programs.values() for Operation in Operations}))
    ...
    SourceLines.extend([')', '', '# each stream program contains only typed field values', 'StreamPrograms = {'])
    for StreamName, Operations in Programs.items():
        ...
        SourceLines.append('    ),')
    SourceLines.extend(['}', '', '# primitive formats keep signed and floating fields faithful to their reader', 'PrimitiveFormats = ' + repr(KPrimFormats), '', '', '# each operation serializes one recovered value through its typed contract', 'def EncodeField(KindName: str, FieldValue: Any) -> bytes:', "    if KindName == 'definition':", '        ClassName, SchemaCode = FieldValue', '        return encode_class_definition(ClassName, SchemaCode)', "    if KindName == 'classref':", '        return encode_class_reference(FieldValue)', "    if KindName == 'objectref':", '        return encode_object_reference(FieldValue)', "    if KindName == 'null':", "        return struct.pack('<H', 0)", "    if KindName == 'string':", '        return encode_string(FieldValue)', "    if KindName == 'stringlist':", "        return struct.pack('<H', len(FieldValue)) + b''.join(encode_string(ItemText) for ItemText in FieldValue)", "    if KindName.startswith('primitive:'):", "        TypeName = KindName.split(':', 1)[1]", "        return struct.pack('<' + PrimitiveFormats[TypeName], FieldValue)", "    if KindName.startswith('direct:'):", "        FormatText = KindName.split(':', 1)[1]", '        ValuesList = FieldValue if isinstance(FieldValue, tuple) else (FieldValue,)', "        return struct.pack('<' + FormatText, *ValuesList)", "    raise SldprtFormatError(f'unknown assembly operation {KindName!r}')", '', '', '# callers may replace semantic fields while source offsets preserve field order', 'def EncodeProgram(StreamName: str, Overrides: Mapping[int, Any] | None = None) -> bytes:', '    try:', '        Operations = StreamPrograms[StreamName]', '    except KeyError as ErrorData:', "        raise SldprtFormatError(f'unknown assembly stream {StreamName!r}') from ErrorData", '    FieldOverrides = Overrides or {}', '    OutputData = bytearray()', '    SourceCursor = 0', '    for StartPos, FieldWidth, OwnerIndex, KindName, DefaultValue in Operations:', '        if StartPos != SourceCursor:', "            raise SldprtFormatError(f'assembly field program drifted at {StartPos}')", '        SourceCursor += FieldWidth', '        FieldValue = FieldOverrides.get(StartPos, DefaultValue)', '        FieldData = EncodeField(KindName, FieldValue)', "        if KindName not in {'string', 'stringlist'} and len(FieldData) != FieldWidth:", "            raise SldprtFormatError(f'assembly field width changed at {StartPos}')", '        OutputData.extend(FieldData)', '    return bytes(OutputData)', ''])
    return '\n'.join(SourceLines)
...
def RunMain() -> int:
    ...
    print(JsonData.dumps({StreamName: {'bytes': len(Archive.require(StreamName)), 'operations': len(Operations)} for StreamName, Operations in Programs.items()}, indent=2))
    return 0
