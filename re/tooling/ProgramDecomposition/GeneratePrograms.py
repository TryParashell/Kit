...
from ProgramRenderer import RenderManifest, RenderMethod, RenderOwner, RenderRegistry, RewriteFacade
...
def ParseArgs() -> Argparse.Namespace:
    ...
    ParserData.add_argument('--program-root', type=PathInfo, default=KProgramRoot, dest='ProgramRoot')
    ParserData.add_argument('--manifest', type=PathInfo, default=KManifestPath, dest='ManifestPath')
...
def CheckPaths(OutputTexts: dict[PathInfo, str]) -> None:
    ...
    for RelativePath in OutputTexts:
        ...
        if PriorPath is not None and PriorPath != RelativePath:
            raise ValueError(f'case insensitive output collision {PriorPath} and {RelativePath}')
...
def BuildOutputs(Programs: tuple[ProgramData, ...]) -> tuple[dict[PathInfo, str], str]:
    ...
    for ProgramItem in Programs:
        ...
        for MethodItem in MethodItems:
            RelativePath = PathInfo(ProgramItem.VariantPath) / 'Methods' / PathInfo(MethodItem.GroupPath + '.py')
            OwnerModule = 'convert.adapters.solidworks.programs.Owners.' + MethodItem.GroupPath.replace('/', '.')
            OutputTexts[RelativePath] = RenderMethod(OwnerModule, MethodItem)
            ModulePaths.append('convert.adapters.solidworks.programs.' + ProgramItem.VariantPath.replace('/', '.') + '.Methods.' + MethodItem.GroupPath.replace('/', '.'))
    ...
    GlobalStats = (len(Programs), sum((len(ProgramItem.Streams) for ProgramItem in Programs)), len(Catalogs), sum((len(OwnerSites) for OwnerSites in Catalogs.values())), MethodTotal, sum((len(Operations) for ProgramItem in Programs for SpareValue, Operations in ProgramItem.Streams)))
...
# needed to keep reverse engineering responsibilities isolated and maintainable
def WriteTree(ProgramRoot: PathInfo, ManifestPath: PathInfo, OutputTexts: dict[PathInfo, str], ManifestText: str) -> None:
...
def ClearModules() -> None:
    ...
    StaleNames = tuple((ModuleName for ModuleName in System.modules if ModuleName.startswith(PrefixText)))
