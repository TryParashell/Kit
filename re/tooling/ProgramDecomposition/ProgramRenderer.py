...
def RenderValue(FieldValue: AnyInfo) -> str:
    ...
    if isinstance(FieldValue, dict):
        PairTexts = (f'{RenderValue(KeyValue)}: {RenderValue(ItemValue)}' for KeyValue, ItemValue in FieldValue.items())
...
def RenderOwner(OwnerSites: tuple[tuple[object, str], ...]) -> str:
    SourceLines = [KHeaderText.rstrip(), '', 'from __future__ import annotations', '', '', '# exact trace spellings stay centralized so variant tables never duplicate owners', 'KOwnerSites = {']
    SourceLines.extend((f'    {RenderValue(OwnerKey)}: {OwnerText!r},' for OwnerKey, OwnerText in OwnerSites))
    SourceLines.extend(('}', ''))
    return FormatSource('\n'.join(SourceLines))
...
def RenderMethod(OwnerModule: str, MethodData: MethodData) -> str:
    SourceLines = [KHeaderText.rstrip(), '', 'from __future__ import annotations', '', f'from {OwnerModule} import KOwnerSites', '', '', '# isolated method data lets new reverse engineered serializers compose independently', 'KMethodProgram = (', '    KOwnerSites,', '    {']
    for StreamName, Operations in MethodData.StreamOps:
        SourceLines.append(f'        {StreamName!r}: (')
        SourceLines.extend(('            ' + RenderValue((StartPos, FieldWidth, OwnerKey, KindName, DefaultValue)) + ',' for StartPos, FieldWidth, OwnerKey, KindName, DefaultValue in Operations))
        SourceLines.append('        ),')
    SourceLines.extend(('    },', ')', ''))
    return FormatSource('\n'.join(SourceLines))
...
def RenderRegistry(ProgramData: ProgramData, ModulePaths: tuple[str, ...]) -> str:
    BuildName = 'BuildStreams' if ProgramData.OpsName == 'StreamPrograms' else 'BuildProgram'
    SourceLines = [KHeaderText.rstrip(), '', 'from __future__ import annotations', '', f'from convert.adapters.solidworks.programs.Common.ProgramComposer import {BuildName}']
    AliasNames: list[str] = []
    ...
    SourceLines.extend(('', '', '# explicit ordering keeps generated imports deterministic while offsets govern composition', 'KMethodPrograms = ('))
    SourceLines.extend((f'    {AliasName},' for AliasName in AliasNames))
    SourceLines.extend((')', '', '', '# compatibility tables preserve every established public import after decomposition'))
    if ProgramData.OpsName == 'StreamPrograms':
        StreamNames = tuple((StreamName for StreamName, SpareValue in ProgramData.Streams))
        SourceLines.append(f'{ProgramData.OwnerName}, {ProgramData.OpsName} = BuildStreams(')
        SourceLines.append('    KMethodPrograms,')
        SourceLines.append(f'    {RenderValue(StreamNames)},')
        SourceLines.append(')')
    else:
        StreamName = ProgramData.Streams[0][0]
        SourceLines.append(f'{ProgramData.OwnerName}, {ProgramData.OpsName} = BuildProgram(')
        SourceLines.append('    KMethodPrograms,')
        SourceLines.append(f'    {StreamName!r},')
        SourceLines.append(')')
    SourceLines.append('')
...
def RewriteFacade(ProgramData: ProgramData) -> str:
    ...
    for NodeData in TreeData.body:
        if isinstance(NodeData, AstLib.Assign):
            NameTexts = {TargetNode.id for TargetNode in NodeData.targets if isinstance(TargetNode, AstLib.Name)}
...
# needed to keep reverse engineering responsibilities isolated and maintainable
def RenderManifest(Programs: tuple[ProgramData, ...], FacadeTexts: dict[str, str], GlobalStats: tuple[int, int, int, int, int, int]) -> str:
    SourceLines = [KHeaderText.rstrip(), '', 'from __future__ import annotations', '', '', '# global counts catch missing variants streams owners methods or recovered operations', f'KGlobalStats = {RenderValue(GlobalStats)}', '', '', '# variant evidence preserves public surfaces logical tables and encoded byte identities', 'KProgramStats = (']
    for ProgramItem in Programs:
        StatRecord = (ProgramItem.VariantPath, ProgramItem.OwnerName, ProgramItem.OpsName, tuple((StreamName for StreamName, SpareValue in ProgramItem.Streams)), sum((len(Operations) for SpareValue, Operations in ProgramItem.Streams)), HashProgram(ProgramItem), ProgramItem.PublicNames, ProgramItem.ByteStats, HashText(FacadeTexts[ProgramItem.VariantPath]))
