...
$log = Join-Path $root "logs\dump_para417.log"
...
& $headless (Join-Path $root "proj_sldmodu") sldmodu `
    ...
    -postScript DumpFunctions.java (Join-Path $root "out\para417_serialize.c") (Join-Path $repo "re\tooling\ghidra\Specs\SpecParaFourOneSevenSerialize.txt") 1 300 `
    -postScript DumpFunctions.java (Join-Path $root "out\para417_accessors.c") (Join-Path $repo "re\tooling\ghidra\Specs\SpecParaFourOneSevenAccessors.txt") 0 120 `
