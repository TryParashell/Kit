...
def CheckProtocols() -> None:
    ...
    assert str(Inspect.signature(CadReaderAdapter.read)) == "(self, source: 'Source', options: 'ReadOptions | None' = None) -> 'CadDocument'"
    assert str(Inspect.signature(CadWriterAdapter.write)) == "(self, document: 'CadDocument', destination: 'Destination', options: 'WriteOptions | None' = None) -> 'WriteResult'"
...
def CheckInfo() -> None:
    ...
    assert FieldNames == ('format_id', 'name', 'version', 'extensions', 'aliases', 'capabilities', 'media_types', 'native_capabilities', 'part_extensions', 'assembly_extensions')
    assert str(Inspect.signature(AdapterInfo)) == "(format_id: 'str', name: 'str', version: 'str', extensions: 'tuple[str, ...]', aliases: 'tuple[str, ...]' = (), capabilities: 'frozenset[Capability]' = frozenset(), media_types: 'tuple[str, ...]' = (), native_capabilities: 'frozenset[Capability]' = frozenset(), part_extensions: 'tuple[str, ...]' = (), assembly_extensions: 'tuple[str, ...]' = ()) -> None"
    ...
    assert DataClasses.replace(InfoData, format_id='format.other').format_id == 'format.other'
    assert str(Inspect.signature(AdapterInfo.__init__)) == "(self, format_id: 'str', name: 'str', version: 'str', extensions: 'tuple[str, ...]', aliases: 'tuple[str, ...]' = (), capabilities: 'frozenset[Capability]' = frozenset(), media_types: 'tuple[str, ...]' = (), native_capabilities: 'frozenset[Capability]' = frozenset(), part_extensions: 'tuple[str, ...]' = (), assembly_extensions: 'tuple[str, ...]' = ()) -> None"
    assert str(Inspect.signature(AdapterInfo.extensions_for)) == "(self, *, assembly: 'bool') -> 'tuple[str, ...]'"
    ...
    with Pytest.raises(TypeError):
        AdapterInfo('format.test', 'Test', '1', ('.test',), FormatId='other', format_id='other')
    with Pytest.raises(TypeError):
...
def CheckBinding() -> None:
    ...
    assert str(Inspect.signature(AdapterBinding)) == "(reader: 'CadReaderAdapter | None' = None, writer: 'CadWriterAdapter | None' = None) -> None"
    assert tuple((FieldData.name for FieldData in DataClasses.fields(AdapterBinding))) == ('reader', 'writer')
    ...
    assert str(Inspect.signature(AdapterBinding.__init__)) == "(self, reader: 'CadReaderAdapter | None' = None, writer: 'CadWriterAdapter | None' = None) -> None"
    with Pytest.raises(TypeError):
...
def CheckErrors() -> None:
    LossError = CapabilityLossError(format_id='format.test', dropped=frozenset({Capability.BREP}))
...
def CheckRegistry() -> None:
    ...
    for MethodName in ('register_reader', 'register_writer', 'register', 'read', 'read_with_adapter', 'write', 'extend'):
    ...
    with Pytest.raises(TypeError):
        RegistryData.read(b'{}', format_id='interchange.json', FormatId='interchange.json')
    with Pytest.raises(TypeError):
...
def CheckExports() -> None:
    ...
    assert set(AdapterPackage.__all__) == {'AdapterBinding', 'AdapterDiscoveryError', 'AdapterInfo', 'AdapterNotFoundError', 'AdapterRegistry', 'AdapterRegistryError', 'AmbiguousAdapterError', 'ApplicationUsabilityError', 'CadAdapter', 'CadReaderAdapter', 'CadWriterAdapter', 'CapabilityLossError', 'CapabilityTransfer', 'CarrierReason', 'Destination', 'ProbeResult', 'ReadOptions', 'Source', 'TransferMode', 'WriteOptions', 'WriteResult', 'is_windows_device_name'}
