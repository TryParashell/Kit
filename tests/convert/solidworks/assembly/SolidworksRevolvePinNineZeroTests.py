...
from convert.adapters.solidworks.programs.configuration.revolve.pin.rightangle.Program import EncodeProgram as EncodeConfig
from convert.adapters.solidworks.programs.configuration.revolve.pin.rightangle.Program import GetCoverage as GetConfigCoverage
from convert.adapters.solidworks.programs.configuration.revolve.pin.rightangle.Program import KConfigOps, KFieldOwners as KConfigOwners, KReferenceDigest as KConfigDigest, KReferenceLength as KConfigLength
from convert.adapters.solidworks.programs.resolved.revolve.pin.rightangle.Program import EncodeProgram, GetCoverage, KAngleOffsets, KFieldOwners, KProfileOffsets, KReferenceDigest, KReferenceLength, KResolvedOps
from convert.adapters.solidworks.container.Format import CONFIGURATION_STREAM as Stream, KIT_RESOLVED_STREAM as StreamA, RESOLVED_FEATURES_STREAM as StreamB
from convert.adapters.solidworks.envelopes.revolve.pin.rightangle.Envelope import BuildEnvelope, CalcPin90Bounds as CalcPinNineZeroBounds, EncodeConfig as EncodeEnvelopeConfig, EncodeFeatures, EncodeHeader, KHeaderIdentity, KHeaderStamps, KHeaderUser
...
# centralizes shared evidence so every related assertion uses one value
KSourcePath = FilePath(__file__).parents[4] / '.rescratch' / 'sw' / 'fcstd' / 'kit_revolve_pin_top_90.FCStd'

# keeps this focused behavior isolated so regressions remain immediately visible
@PytestLib.mark.skipif(not KSourcePath.is_file(), reason='top plane partial pin revolution corpus unavailable')
...
def TestPNZCPHETC() -> None:
    ...
    assert CoverageData == {'stream_bytes': 24902, 'typed': 24902, 'opaque': 0, 'accounted': 24902, 'operations': 4297, 'owners': 1058}
...
def TestPNZMHBFP() -> None:
    ...
    assert MathInfo.isclose(HeaderBounds[9], 0.025062422069704278, rel_tol=0.0, abs_tol=1e-18)
...
def TestPNZPHETC() -> None:
    ...
    assert CoverageData == {'stream_bytes': 12537, 'typed': 12537, 'opaque': 0, 'accounted': 12537, 'operations': 3073, 'owners': 506}
    assert b'C:\\Users' not in PayloadData
    assert b'.rescratch' not in PayloadData
...
def TestPNZPCBAV() -> None:
    ...
    assert all((PayloadData[OffsetValue:OffsetValue + 8] == AngleBytes for OffsetValue in KAngleOffsets))
...
def TestPNZPASWAO() -> None:
    ...
    assert all((StructLib.unpack_from('<d', PayloadData, OffsetValue)[0] == AngleValue for OffsetValue in KAngleOffsets))
