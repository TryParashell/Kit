...
from convert.adapters.solidworks.programs.configuration.revolve.pin.midplane.Program import EncodeProgram as EncodeConfig
from convert.adapters.solidworks.programs.configuration.revolve.pin.midplane.Program import GetCoverage as GetConfigCoverage
from convert.adapters.solidworks.programs.configuration.revolve.pin.midplane.Program import KConfigOps, KFieldOwners as KConfigOwners, KReferenceDigest as KConfigDigest, KReferenceLength as KConfigLength
from convert.adapters.solidworks.container.Container import SldprtArchive, SldprtFormatError
from convert.adapters.solidworks.container.Format import CONFIGURATION_STREAM as Stream, RESOLVED_FEATURES_STREAM as StreamA
from convert.adapters.solidworks.programs.resolved.revolve.pin.midplane.Program import EncodeProgram, GetCoverage, KFieldOwners, KFirstAngleOffsets, KProfileOffsets, KReferenceDigest, KReferenceLength, KResolvedOps, KSecondAngleOffsets, KSingleEndOffset
from convert.adapters.solidworks.envelopes.revolve.pin.default.Envelope import CalcPinBounds
from convert.adapters.solidworks.envelopes.revolve.pin.midplane.Envelope import BuildEnvelope, EncodeConfig as EncodeEnvelopeConfig, EncodeFeatures, EncodeHeader, KHeaderIdentity, KHeaderStamps, KHeaderUser

# centralizes shared evidence so every related assertion uses one value
KProfileMetres = ((0.0, -0.05), (0.0, 0.0), (0.0025, 0.0), (0.0025, -0.03), (0.0015, -0.02999), (0.0015, -0.05))

# centralizes shared evidence so every related assertion uses one value
KHeaderDigest = '4cb455532120074010565342eab6df3b83df1bf45ad0c25bf391664790de07ca'

# centralizes shared evidence so every related assertion uses one value
KSourcePath = FilePath(__file__).parents[4] / '.rescratch' / 'sw' / 'fcstd' / 'kit_revolve_pin_top_midplane.FCStd'

# centralizes shared evidence so every related assertion uses one value
KOraclePath = FilePath(__file__).parents[4] / '.rescratch' / 'revolve_pin_midplane' / 'revolve_pin_top_midplane.oracle.SLDPRT'

# keeps this focused behavior isolated so regressions remain immediately visible
@PytestLib.mark.skipif(not KSourcePath.is_file(), reason='top plane symmetric pin revolution corpus unavailable')
def TestMPPWRFC(TmpPath: FilePath) -> None:
    SourceData = ReadFreecad(KSourcePath)
    with PytestLib.raises(ApplicationUsabilityError):
        WriteDocument(SourceData, TmpPath / 'MidplanePinPending.SLDPRT', allow_carrier=False)
...
def TestMPCPHETC() -> None:
    ...
    assert GetConfigCoverage() == {'stream_bytes': 24902, 'typed': 24902, 'opaque': 0, 'accounted': 24902, 'operations': 4297, 'owners': 1058}
...
def TestMPPHETC() -> None:
    ...
    assert GetCoverage() == {'stream_bytes': 14065, 'typed': 14065, 'opaque': 0, 'accounted': 14065, 'operations': 3374, 'owners': 506}
    assert b'C:\\Users' not in PayloadData
    assert b'.rescratch' not in PayloadData
...
def TestMPPOTWG() -> None:
    ...
    for StartPos, FieldWidth, OwnerIndex, KindName, DefaultValue in KResolvedOps:
        ...
        assert KindName in AllowedKinds or KindName.startswith('primitive:') or KindName.startswith('direct:')
        CursorValue += FieldWidth
...
def TestMPPCBEV() -> None:
    ...
    assert all((StructLib.unpack_from('<d', PayloadData, OffsetValue)[0] == MathInfo.tau for OffsetValue in KFirstAngleOffsets))
    assert all((StructLib.unpack_from('<d', PayloadData, OffsetValue)[0] == 0.0 for OffsetValue in KSecondAngleOffsets))

# keeps this focused behavior isolated so regressions remain immediately visible
def TestMPPCAPV() -> None:
    PayloadData = EncodeProgram()
    assert tuple(((StructLib.unpack_from('<d', PayloadData, XOffset)[0], StructLib.unpack_from('<d', PayloadData, YOffset)[0]) for XOffset, YOffset in KProfileOffsets)) == KProfileMetres
...
def TestMPPAIFAO() -> None:
    ...
    assert all((StructLib.unpack_from('<d', PayloadData, OffsetValue)[0] == AngleValue for OffsetValue in KFirstAngleOffsets))
    assert all((StructLib.unpack_from('<d', PayloadData, OffsetValue)[0] == 0.0 for OffsetValue in KSecondAngleOffsets))
