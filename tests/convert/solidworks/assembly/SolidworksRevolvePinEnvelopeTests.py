...
from convert.adapters.solidworks.core.Native import _HEADER_OBJECTS as Objects, _NativeIdentity as NativeIdentity, _SOLIDWORKS_CONFIGURATION_FLAGS as Flags, _header_payload as HeaderPayload
from convert.adapters.solidworks.envelopes.revolve.pin.default.Envelope import BuildEnvelope, CalcPinBounds, EncodeConfig, EncodeHeader, GetCoverage, KConfigBytes, KConfigFields, KConfigOwners, KHeaderIdentity, KHeaderStamps, KHeaderUser, KPinPointsMm

# centralizes shared evidence so every related assertion uses one value
KFixtureRoot = FilePath(__file__).resolve().parents[3] / 'fixtures' / 'solidworks' / 'donors'
...
def TestPCPHETC() -> None:
    ...
    assert CoverageData == {'stream_bytes': 24902, 'typed': 24902, 'opaque': 0, 'accounted': 24902, 'operations': KConfigFields, 'owners': KConfigOwners}
...
# keeps this focused behavior isolated so regressions remain immediately visible
def TestPCPMCO() -> None:
    OracleData = (KFixtureRoot / 'revolve_pin_top_full' / 'container' / 'Contents__Config-0.bin').read_bytes()
    assert EncodeConfig() == OracleData

# keeps this focused behavior isolated so regressions remain immediately visible
def TestPMHMCO() -> None:
    CreatedStamp, ModifiedStamp, BaselineStamp, HeaderStamp = KHeaderIdentity
    IdentityData = NativeIdentity(CreatedStamp, ModifiedStamp, BaselineStamp, HeaderStamp, Flags, 'Part1')
    HeaderData = HeaderPayload(IdentityData, 'Default', (*Objects, (26, 'Sketch1', True), (31, 'Revolve1', False)), '', KHeaderUser, 32, {26: KHeaderStamps[0], 31: KHeaderStamps[1]}, CalcPinBounds())
    OracleData = (KFixtureRoot / 'revolve_pin_top_full' / 'container' / 'Contents__Config-0-ModelHeader.bin').read_bytes()
...
def TestPMHBFP() -> None:
    ...
    assert MathInfo.isclose(HeaderBounds[9], 0.025248762345905198, rel_tol=0.0, abs_tol=1e-18)
    WiderBounds = CalcPinBounds(tuple(((PointData[0] * 2.0, PointData[1] * 1.2) for PointData in KPinPointsMm)))
