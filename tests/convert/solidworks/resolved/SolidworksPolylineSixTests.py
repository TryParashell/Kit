...
from convert.adapters.solidworks.container.Format import CONFIGURATION_STREAM as Stream, KEYWORDS_STREAM as StreamA, RESOLVED_FEATURES_STREAM as StreamB
from convert.adapters.solidworks.core.Native import HasVendorPartEncoding, decode_native_model as DecodeNativeModel
...
# centralizes shared evidence so every related assertion uses one value
KDonorStream = KRepoRoot / 'tests' / 'fixtures' / 'solidworks' / 'donors' / 'poly6_boss' / 'resolved.bin'

# centralizes shared evidence so every related assertion uses one value
KGateDigest = 'b973bd5326bbdb65b8e8b5e8345e0bdbdef20d345bf70d9f7562e5a74077bfb4'

# centralizes shared evidence so every related assertion uses one value
KDonorPoints = ((0.0, 0.0), (40.0, 0.0), (40.0, 10.0), (15.0, 10.0), (15.0, 25.0), (0.0, 25.0))
...
def TestPSPVIS() -> None:
    ...
    with PytestLib.raises(SldprtFormatError, match='intersect'):
        PadFieldMap(((0.0, 0.0), (2.0, 2.0), (0.0, 2.0), (2.0, 0.0), (3.0, 0.0), (3.0, 3.0)), 8.0)
...
def TestPSPOANF() -> None:
    ...
    assert tuple((FieldValues[OffsetPos] for OffsetPos in KPointOffsets)) == (0.0, 0.0, 0.04, 0.0, 0.04, 0.01, 0.015, 0.01, 0.015, 0.025, 0.0, 0.025)
    assert FieldValues[KDepthOffset] == 0.008

# keeps this focused behavior isolated so regressions remain immediately visible
@PytestLib.mark.skipif(not KFreeCadPolyline.is_file(), reason='six line FreeCAD corpus unavailable')
def TestFCPSWNPSWP(TmpPath: FilePath) -> None:
    ...
    NativeData = DecodeNativeModel(ArchiveData.require(StreamA), ResolvedData, ArchiveData.require(Stream), resolved_stream=StreamB)
    ...
    assert tuple(((ItemData.kind, ItemData.coordinates) for ItemData in NativeData.sketches[0].profiles)) == (('polyline', (-20.0, -20.0, 20.0, -20.0, 20.0, 0.0, 0.0, 0.0, 0.0, 20.0, -20.0, 20.0)),)
...
def TestFCPSROSC() -> None:
    ...
    FiveLineData = ReplaceData(SourceData, sketches=(ReplaceData(SketchData, entities=SketchData.entities[:-1], closed_profile_entity_ids=(ProfileData[:-1],)),))
