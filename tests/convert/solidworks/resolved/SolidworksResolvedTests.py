...
from convert.adapters.solidworks.resolved.Core import BLIND_END_CONDITION as Condition, BOSS_FLAGS as Flags, BOSS_KIND as KindInfo, CIRCLE_POINT_ANGLE_DEGREES as Degrees, CUT_FLAGS as FlagsA, CUT_KIND as KindInfoA, FEATURE_FLAGS_MASK as MaskInfo, FEATURE_KIND_BY_FLAGS as FlagsB, FIRST_FEATURE_END_CONDITION_DISTANCE as Distance, FIRST_FEATURE_REVERSE_DISTANCE as DistanceA, FULL_CIRCLE_DEGREES as DegreesA, LATER_FEATURE_END_CONDITION_DISTANCE as DistanceB, LATER_FEATURE_REVERSE_DISTANCE as DistanceC, LOFT_FLAGS as FlagsC, LOFT_KIND as KindInfoB, MID_PLANE_END_CONDITION as ConditionA, PLANE_FLAGS as FlagsD, ROUND_FLAGS as FlagsE, ROUND_KIND as KindInfoC, SKETCH_FLAGS as FlagsF, SKETCH_ON_CURVE_ROLE as RoleInfo, SKETCH_POINT_CLASS as Class, SWEEP_FLAGS as FlagsG, SWEEP_KIND as KindInfoD, SWEEP_SINGLE_PROFILE_FLAGS as FlagsH, TREE_NODE_FLAGS as FlagsI, FeatureEdit, PatchSketchPlane, circle_circumference_point_mm as CircleCPM, circle_radius_mm as CircleRadiusMm, class_records as ClassRecords, dimension_scalars as DimensionScalars, feature_kind as FeatureKind, is_tree_node_flags as IsTreeNodeFlags, locate_features as LocateFeatures, locate_rectangle_pad as LocateRectanglePad, name_records as NameRecords, patch_features as PatchFeatures, patch_sketch_arcs as PatchSketchArcs, rectangle_corners_mm as RectangleCornersMm, sketch_arcs as SketchArcs, sketch_coordinates as SketchCoordinates, sketch_plane_object_id as SketchPlaneObjectId, sketch_points as SketchPoints, tree_nodes as TreeNodes
...
from convert.adapters.solidworks.programs.resolved.boss.cut.triple.Program import EncodeProgram as EncodeBCCCP
from convert.adapters.solidworks.programs.resolved.boss.cut.through.Program import EncodeProgram as EncodeBCTP
...
from convert.adapters.solidworks.programs.resolved.boss.pattern.circular.Program import EncodeProgram as EncodeBCPP
from convert.adapters.solidworks.programs.resolved.boss.fillet.Program import EncodeProgram as EncodeBossFilletProgram
from convert.adapters.solidworks.programs.resolved.boss.pattern.linear.Program import EncodeProgram as EncodeBLPP
...
# centralizes shared evidence so every related assertion uses one value
KExtrusions = ((35, KindInfo, 38.0), (188, KindInfo, 18.0), (204, KindInfo, 30.7), (214, KindInfoA, 46.7), (228, KindInfoA, 5.0), (250, KindInfoA, 9.0))

# centralizes shared evidence so every related assertion uses one value
KChamfers = ((231, 2.0), (236, 1.0), (253, 1.0), (256, 2.0))

# centralizes shared evidence so every related assertion uses one value
KFeaturesA = {45: KindInfoB, 48: KindInfoB, 64: KindInfoD, 189: KindInfoD, 210: KindInfoD, 234: KindInfoD, 240: KindInfoD}
...
# centralizes shared evidence so every related assertion uses one value
KConditions = {'CUTBASE_cd5': ((False, Condition), (True, Condition)), 'CUTMID_d5': ((False, Condition), (True, ConditionA)), 'CUTFACE_d5': ((False, Condition), (False, Condition)), 'PADPLANE_rev_d5': ((False, Condition), (True, Condition)), 'TWOPAD_d5': ((False, Condition), (False, Condition)), 'CIRCLECUT_r4': ((False, Condition), (True, Condition)), 'THREEFEATURE_pad_cut_pad': ((False, Condition), (True, Condition), (False, Condition))}

# centralizes shared evidence so every related assertion uses one value
KTrips = (('A_cutbase_both_features', 'CUTBASE_cd5', {0: (50.0, 30.0, 12.0), 1: (14.0, 14.0, 7.0)}), ('B_cutbase_second_feature_only', 'CUTBASE_cd5', {1: (8.0, 8.0, 3.0)}), ('C_twopad_both_features', 'TWOPAD_d5', {0: (45.0, 25.0, 11.0), 1: (12.0, 12.0, 6.0)}), ('D_threefeature_all_three', 'THREEFEATURE_pad_cut_pad', {0: (60.0, 25.0, 14.0), 1: (12.0, 12.0, 6.0), 2: (6.0, 6.0, 3.0)}))
...
def AuthoredParts() -> tuple[FilePath, ...]:
    return tuple(sorted((TargetPath for TargetPath in KCorpus.glob('*.SLDPRT') if not TargetPath.name.startswith('~$'))))
...
def AuthoredNodes(KeywordsA: ElementTree.Element) -> dict[int, ElementTree.Element]:
    return {int(Element.get('id', '-1')): Element for Element in KeywordsA if Element.get('id', '').isdigit()}
...
def AuthoredRadiiMm(Element: ElementTree.Element) -> tuple[float, ...]:
    ...
    for Dimension in Element:
        ...
        elif TextInfo.startswith(KPrefixA) and TextInfo[len(KPrefixA):].replace('.', '', 1).isdigit():
            Radii.append(float(TextInfo[len(KPrefixA):]))
    return tuple(Radii)

# keeps this focused behavior isolated so regressions remain immediately visible
def AuthoredRBF(KeywordsA: ElementTree.Element) -> tuple[float, ...]:
    return tuple((Radius for Element in KeywordsA for Radius in AuthoredRadiiMm(Element)))
...
def CentredCorners(WidthMm: float, HeightMm: float) -> tuple[tuple[float, float], ...]:
    return RectangleCornersMm(-WidthMm / 2.0, -HeightMm / 2.0, WidthMm / 2.0, HeightMm / 2.0)
...
def TestFFWHODAMTK() -> None:
    ...
    assert dict(FlagsB) == {Flags: KindInfo, FlagsA: KindInfoA, FlagsE: KindInfoC, FlagsG: KindInfoD, FlagsH: KindInfoD, FlagsC: KindInfoB}
...
def TestRCOITPS() -> None:
    assert RectangleCornersMm(-20.0, -10.0, 20.0, 10.0) == ((-20.0, -10.0), (20.0, 10.0), (-20.0, 10.0), (20.0, -10.0))
...
def TestWRPIRTTGL() -> None:
    ...
    assert tuple((Point.offset for Point in Feature.points)) == tuple((XOffset for XOffset, IgnoredValue in Layout.point_offsets))
...
def TestNRETTNADS() -> None:
    ...
    assert {NodeInfo.name for NodeInfo in Nodes} >= {'Sketch1', 'Boss-Extrude1', 'Sketch2', 'Cut-Extrude1'}
    assert {NodeInfo.offset for NodeInfo in Nodes} <= {RecordInfo.offset for RecordInfo in RecordList}
    assert [NodeInfo.flags for NodeInfo in Nodes if NodeInfo.name == 'Boss-Extrude1'] == [Flags]
    assert [NodeInfo.flags for NodeInfo in Nodes if NodeInfo.name == 'Cut-Extrude1'] == [FlagsA]
    assert [NodeInfo.flags for NodeInfo in Nodes if NodeInfo.name == 'Sketch1'] == [FlagsF]
    assert [NodeInfo.flags for NodeInfo in Nodes if NodeInfo.name == 'Sketch2'] == [FlagsF]
    Scalars = DimensionScalars(Resolved)
    Depths = [Scalar for Scalar in Scalars if Scalar.name.startswith('D')]
    assert [Scalar.value_mm for Scalar in Depths] == [PytestLib.approx(10.0), PytestLib.approx(5.0)]
...
def TestPFRAWRP() -> None:
    ...
    Patched = PatchFeatures(Resolved, {0: FeatureEdit(corners_mm=Corners, depth_mm=7.5, reversed=True, end_condition_code=ConditionA, update_depth_copies=True)})
...
def TestPFRETSCH() -> None:
    Resolved = RectanglePS()
    Rejected = ({9: FeatureEdit(depth_mm=5.0)}, {0: FeatureEdit(corners_mm=((0.0, 0.0),))}, {0: FeatureEdit(corners_mm=CentredCorners(MathInfo.inf, 4.0))}, {0: FeatureEdit(depth_mm=0.0)}, {0: FeatureEdit(depth_mm=MathInfo.nan)}, {0: FeatureEdit(end_condition_code=1)}, {0: FeatureEdit(update_depth_copies=True)})
...
def TestDBPPEDD() -> None:
    ...
    PatchedData = PatchFeatures(ProgramData, {0: FeatureEdit(corners_mm=RectangleCornersMm(0.0, 0.0, 20.0, 15.0), depth_mm=12.0, update_depth_copies=True, SketchDimensionsMm=(20.0, 15.0))})
...
def TestDBPRIDD() -> None:
    ...
    for DimensionData in ((10.0,), (10.0, 0.0), (10.0, MathInfo.nan)):
        with PytestLib.raises(SldprtFormatError):
            PatchFeatures(ProgramData, {0: FeatureEdit(SketchDimensionsMm=DimensionData)})
...
def TestFPSPPCDT() -> None:
    ...
    assert (len(BaselineData), len(CircleData), len(TopData), len(RightData)) == (11075, 12514, 11075, 11147)
...
def TestFPPPPCBOA() -> None:
    for ProgramData, ExpectedLength in ((EncodeCutBaseProgram(), 16579), (EncodeBossCutProgram(), 16472), (EncodeBCTP(), 14693)):
...
def TestFPBFPIC() -> None:
    ...
    NamedData = [ItemData for ItemData in NameRecords(ProgramData) if ItemData.name in {'Sketch1', 'Boss-Extrude1', 'Fillet1'}]
    ScalarData = DimensionScalars(ProgramData)
    assert len(ProgramData) == 14962
    assert Hashlib.sha256(ProgramData).hexdigest() == 'fed3e9464e9fa26722941b7355502fe2d3c24243bc00993f22726f39d11418bb'
    assert [(ItemData.name, ItemData.feature_id) for ItemData in NamedData] == [('Sketch1', 26), ('Boss-Extrude1', 32), ('Fillet1', 34)]
    assert [(ItemData.name, ItemData.value_mm) for ItemData in ScalarData[-2:]] == [('D1', 10.0), ('D1', 2.0)]

# keeps this focused behavior isolated so regressions remain immediately visible
def TestBFPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/resolved/boss/fillet/Program.py'
...
def TestFPBCPIC() -> None:
    ...
    NamedData = [ItemData for ItemData in NameRecords(ProgramData) if ItemData.name in {'Sketch1', 'Boss-Extrude1', 'Chamfer1'}]
    ScalarData = DimensionScalars(ProgramData)
    assert len(ProgramData) == 15811
    assert Hashlib.sha256(ProgramData).hexdigest() == 'd8b6f859a0e60e5e6307833ce502723123663bc9e25ca2b46e74f608dd5b9450'
    assert [(ItemData.name, ItemData.feature_id) for ItemData in NamedData] == [('Sketch1', 26), ('Boss-Extrude1', 32), ('Chamfer1', 35)]
    assert [(ItemData.name, ItemData.value_mm) for ItemData in ScalarData] == [('D1', 10.0), ('D1', 2.0), ('D2', PytestLib.approx(MathInfo.pi * 250.0))]

# keeps this focused behavior isolated so regressions remain immediately visible
def TestBCPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/resolved/boss/chamfer/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestFPBSPIC() -> None:
    ...
    NamedData = [ItemData for ItemData in NameRecords(ProgramData) if ItemData.name in {'Sketch1', 'Boss-Extrude1', 'Shell1'}]
    ScalarData = DimensionScalars(ProgramData)
    assert len(ProgramData) == 13868
    assert Hashlib.sha256(ProgramData).hexdigest() == '19572f2d262a02c450ac66315089598074f506880ac0df03f8a74670ffac0191'
    assert [(ItemData.name, ItemData.feature_id) for ItemData in NamedData] == [('Sketch1', 26), ('Boss-Extrude1', 32), ('Shell1', 34)]
    assert [(ItemData.name, ItemData.value_mm) for ItemData in ScalarData] == [('D1', 10.0), ('D1', 2.0)]

# keeps this focused behavior isolated so regressions remain immediately visible
def TestBSPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/resolved/boss/shell/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestFPBLPPIC() -> None:
    ...
    NamedData = [ItemData for ItemData in NameRecords(ProgramData) if ItemData.name in {'Sketch1', 'Boss-Extrude1', 'LPattern1'}]
    ScalarData = DimensionScalars(ProgramData)
    assert len(ProgramData) == 22264
    assert Hashlib.sha256(ProgramData).hexdigest() == 'fa69899e0a0d5f3271f2e1a9fff8e8eae396c7492f8910ef3ba470b3f53bb370'
    assert [(ItemData.name, ItemData.feature_id) for ItemData in NamedData] == [('Sketch1', 26), ('Boss-Extrude1', 32), ('LPattern1', 40)]
    assert ScalarData[0].name == 'D1'
    assert ScalarData[0].value_mm == PytestLib.approx(5.0)
    assert next((ItemData for ItemData in ScalarData if ItemData.name == 'D3')).value_mm == PytestLib.approx(5.0)

# keeps this focused behavior isolated so regressions remain immediately visible
def TestBLPPEPADF() -> None:
    ProgramData = EncodeBLPP({12656: 0.005, 14535: 0.005, 14569: -0.0, 14577: -0.0, 14585: 1.0, 14620: -0.0, 14636: -MathInfo.sqrt(0.5), 14644: -0.0, 14660: MathInfo.sqrt(0.5), 14668: 1.0, 14692: -0.0, 18577: 1})
    ...
    assert StructLib.unpack_from('<d', ProgramData, 14636)[0] == PytestLib.approx(-MathInfo.sqrt(0.5))
    assert StructLib.unpack_from('<d', ProgramData, 14660)[0] == PytestLib.approx(MathInfo.sqrt(0.5))

# keeps this focused behavior isolated so regressions remain immediately visible
def TestBLPPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/resolved/boss/pattern/linear/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestFPBCPPIC() -> None:
    ...
    NamedData = [ItemData for ItemData in NameRecords(ProgramData) if ItemData.name in {'Sketch1', 'Boss-Extrude1', 'CirPattern1'}]
    assert len(ProgramData) == 19603
    assert Hashlib.sha256(ProgramData).hexdigest() == 'ced6aec7dd5b4bc323416dfd89afe75d684aa8ad9010b54438ec516798f91be3'
    assert [(ItemData.name, ItemData.feature_id) for ItemData in NamedData] == [('Sketch1', 26), ('Boss-Extrude1', 32), ('CirPattern1', 46)]
    ...
    assert StructLib.unpack_from('<d', ProgramData, 18584)[0] == PytestLib.approx(MathInfo.tau)
    assert StructLib.unpack_from('<d', ProgramData, 19026)[0] == PytestLib.approx(MathInfo.tau)
    assert StructLib.unpack_from('<d', ProgramData, 19050)[0] == PytestLib.approx(MathInfo.tau)
...
def TestBCPPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/resolved/boss/pattern/circular/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestFPBCCPCTO() -> None:
    ...
    assert [ItemData.kind for ItemData in FeatureData] == [KindInfo, KindInfoA, KindInfoA]
...
def TestFPBCCCPCFO() -> None:
    ...
    assert [ItemData.kind for ItemData in FeatureData] == [KindInfo, KindInfoA, KindInfoA, KindInfoA]
...
def TestCPLEFWIMP(NameText: str) -> None:
    ...
    for Feature, (KindInfoE, DepthMm, PointCount) in zip(Features, Expected, strict=True):
...
def TestTFIRWACM() -> None:
    ...
    assert [Feature.depth_mm for Feature in Features] == [PytestLib.approx(10.0), PytestLib.approx(5.0), PytestLib.approx(4.0)]
...
def TestPFBVATC() -> None:
    ...
    Patched = PatchFeatures(Resolved, {1: FeatureEdit(depth_mm=9.0, reversed=False, end_condition_code=Condition, update_depth_copies=True)})
    assert len(Patched) == len(Resolved)
...
@KCorpusParts
@KCorpusPatched
@PytestLib.mark.parametrize(('name', 'donor', 'spec'), KTrips)
def TestPFRTPRTS(NameText: str, Donor: str, SpecInfo: dict[int, tuple[float, float, float]]) -> None:
    Resolved = CorpusStream(Donor)
    Edits = {Ordinal: FeatureEdit(corners_mm=CentredCorners(WidthMm, HeightMm), depth_mm=DepthMm) for Ordinal, (WidthMm, HeightMm, DepthMm) in SpecInfo.items()}
...
def TestBBACFMTKD() -> None:
    ...
    Extrusions = tuple((Feature for Feature in Features if Feature.kind in {KindInfo, KindInfoA}))
    assert tuple(((Feature.feature_id, Feature.kind) for Feature in Extrusions)) == tuple(((Identifier, KindInfoE) for Identifier, KindInfoE, IgnoredValue in KExtrusions))
    for Feature, (Identifier, IgnoredValue, DepthMm) in zip(Extrusions, KExtrusions, strict=True):
        Element = Authored[Identifier]
        assert Element.tag == 'Extrusion'
        assert float(AuthoredD(Element, 'D1') or 'nan') == PytestLib.approx(DepthMm)
        assert Feature.depth_mm == PytestLib.approx(DepthMm)
        assert Feature.flags & MaskInfo == (Flags if Feature.kind == KindInfo else FlagsA)
...
def TestBCACARF() -> None:
    ...
    assert tuple((Feature.feature_id for Feature in Rounds)) == tuple((Identifier for Identifier, IgnoredValue in KChamfers))
...
def TestTTSALMTKT() -> None:
    ...
    Swept = {Feature.feature_id: Feature for Feature in Features if Feature.kind in {KindInfoD, KindInfoB}}
    assert {Identifier: Feature.kind for Identifier, Feature in Swept.items()} == KFeaturesA
...
def TestBCPDTAD() -> None:
    ...
    Authored = AuthoredNodes(AuthoredK('BIELA'))
    Features = LocateFeatures(Resolved)
    Circular = {Feature.feature_id: Feature.radii_mm for Feature in Features if Feature.arcs}
    assert Circular == {35: (PytestLib.approx(22.2), PytestLib.approx(17.8)), 214: (PytestLib.approx(4.5), PytestLib.approx(4.5)), 250: (PytestLib.approx(1.25),)}
    for Feature in Features:
        ...
        for ArcInfo in Feature.arcs:
            assert any((MathInfo.isclose(ArcInfo.radius_mm, Radius, rel_tol=1e-09, abs_tol=1e-09) for Radius in Expected))
...
def NamedTARAACFBAS() -> None:
    Resolved = AuthoredRS('BIELA')
    Coordinates = {Coordinate.offset: Coordinate for Coordinate in SketchCoordinates(Resolved)}
    for ArcInfo in SketchArcs(Resolved):
        ...
        assert CircleRadiusMm(RimInfo.x_mm - Centre.x_mm, RimInfo.y_mm - Centre.y_mm) == PytestLib.approx(ArcInfo.radius_mm)
...
def TestACCRAWTAD() -> None:
    ...
    for TargetPath in AuthoredParts():
        ...
        for ArcInfo in ArcsInfo:
            ...
            Matched += any((MathInfo.isclose(ArcInfo.radius_mm, Radius, rel_tol=1e-09, abs_tol=1e-09) for Radius in Expected))
    assert Decoded >= 480
...
def TestPFRTROACP() -> None:
    ...
    Patched = PatchFeatures(Resolved, {Circular.ordinal: FeatureEdit(radii_mm=(6.0, 6.5), arc_centres_mm=ExpectedCentres, depth_mm=44.0)})
    assert len(Patched) == len(Resolved)
    ...
    Rejected = ({Circular.ordinal: FeatureEdit(radii_mm=(6.0,))}, {Circular.ordinal: FeatureEdit(radii_mm=(6.0, -1.0))}, {Circular.ordinal: FeatureEdit(radii_mm=(6.0, MathInfo.nan))}, {Features[5].ordinal: FeatureEdit(radii_mm=(6.0,))})
    for Edits in Rejected:
        with PytestLib.raises(SldprtFormatError):
            PatchFeatures(Resolved, Edits)
...
# keeps this focused behavior isolated so regressions remain immediately visible
@KCorpusParts
@PytestLib.mark.parametrize(('name', 'radius_mm'), (('CIRCLECUT_r4', 4.0), ('CIRCLECUT_r6', 6.0)))
def TestCCRIRTLF(NameText: str, RadiusMm: float) -> None:
    ...
    assert Circular.bounds_mm == (PytestLib.approx(-RadiusMm), PytestLib.approx(-RadiusMm), PytestLib.approx(RadiusMm), PytestLib.approx(RadiusMm))
    Patched = PatchFeatures(Resolved, {1: FeatureEdit(radii_mm=(RadiusMm + 1.5,))})
    assert LocateFeatures(Patched)[1].arcs[0].radius_mm == PytestLib.approx(RadiusMm + 1.5)
