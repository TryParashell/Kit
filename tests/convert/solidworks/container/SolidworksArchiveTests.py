...
def MakeLayout(GroupData: dict, ChildSlots=(), RunData=None) -> LayoutTable:
    if RunData is None:
        RunData = {'lead': 0}
    return LayoutTable.from_mapping({'version': 1, 'classes': {'solo': {'confidence': 'partial', 'child_slots': ChildSlots, 'runs': RunData, 'groups': [GroupData]}}})

# keeps this focused behavior isolated so regressions remain immediately visible
def TestRGAV() -> None:
    Sound = {'name': 'loop', 'count': {'back': 2, 'width': 2}, 'slots': ['*'], 'element': [0]}
    assert MakeLayout(Sound)['solo'].walks_groups
    InvalidGroups = ({**Sound, 'name': ''}, {**Sound, 'element': []}, {**Sound, 'element': [-1]}, {**Sound, 'slots': ['*', '*']}, {**Sound, 'trailer': -1}, {**Sound, 'count': {'back': 1, 'width': 2}}, {**Sound, 'count': {'back': 2, 'width': 3}}, {'name': 'loop', 'slots': ['*'], 'element': [0]}, {**Sound, 'repeat': 1}, {'name': 'loop', 'repeat': 0, 'slots': ['*'], 'element': [0]}, {**Sound, 'element_by_version': {'nope': [0]}}, {**Sound, 'element_by_version': {'18000': [0, 0]}}, {**Sound, 'element_run_variants': {}}, {**Sound, 'count_variants': {}}, {**Sound, 'trailer_variants': {}}, {**Sound, 'element_run_variants': [{'slot': 1, 'predicate_at': 0, 'predicate_width': 1, 'values': [1], 'run': 0}]})
