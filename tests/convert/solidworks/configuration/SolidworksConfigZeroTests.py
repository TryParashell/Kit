...
from convert.adapters.solidworks.configuration.ConfigZero import CONFIG_FIELD_COUNT as Count, CONFIG_OPAQUE_BYTES as Bytes, CONFIG_OWNER_COUNT as CountA, FILLET_ANNOTATION_BYTES as BytesA, FILLET_ATOM_LINK_STAMP as Stamp, FILLET_ATOM_LINK_STAMP_RELATIVES as Relatives, FILLET_ATOM_PARENT_RELATIVE as Relative, MO_VERSION as Version, PATTERN_ANNOTATION_BYTES as BytesB, PER_FEATURE_ATOM_BYTES as BytesC, REFERENCE_ATOM_ID as IdInfo, REFERENCE_LENGTH as Length, REFERENCE_SHA256 as ShaTwoFiveSix, REFERENCE_TREE_ID as IdInfoA, SINGLE_LENGTH_UNIT_LENGTH as LengthA, TWO_VIEW_ANNOTATION_BYTES as BytesD, declared_opaque_split as DeclaredOpaqueSplit, encode_config0_stream as EncodeConfigZeroStream
from convert.adapters.solidworks.container.Archive import encode_class_definition as EncodeClassDefinition
from convert.adapters.solidworks.programs.configuration.default.Program import ConfigOps, FieldOwners, ReferenceLength, ShiftMapReference
from convert.adapters.solidworks.programs.configuration.box.Program import ConfigOps as BoxConfigOps, EncodeProgram as EncodeBoxConfigProgram, FieldOwners as BoxFieldOwners, ReferenceLength as BoxReferenceLength
from convert.adapters.solidworks.programs.configuration.views.pair.Program import AnnotationOps, FieldOwners as AnnotationFieldOwners, ReferenceLength as AnnotationReferenceLength
from convert.adapters.solidworks.programs.configuration.fillet.views.Program import AnnotationOps as FilletAnnotationOps, FieldOwners as FilletFieldOwners, ReferenceLength as FilletReferenceLength
from convert.adapters.solidworks.programs.configuration.pattern.views.Program import AnnotationOps as PatternAnnotationOps, FieldOwners as PatternFieldOwners, ReferenceLength as PatternReferenceLength
...
def Atoms(FeatureCount: int) -> tuple[tuple[int, int], ...]:
    return tuple(((IdInfo + Index, IdInfoA + 8 * Index) for Index in range(FeatureCount)))
...
def TestERBHEOTO() -> None:
    ...
    for StartPos, FieldWidth, OwnerIndex, KindName, FieldValue in ConfigOps:
        ...
        assert KindName in {'definition', 'classref', 'objectref', 'null', 'string', 'stringlist'} or KindName.startswith(('primitive:', 'direct:'))
...
def TestRPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/configuration/default/Program.py'
...
def TestDBCIET() -> None:
    ...
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/configuration/box/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestTVRPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/configuration/views/pair/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestTFCITAL() -> None:
    StreamData = EncodeConfigZeroStream(part_name='Part1', atoms=((101, 34),), high_water=(101, 103), annotation_view_count=2, terminal_parent_tree_id=32)
    ...
    assert all((StructLib.unpack_from('<I', StreamData, AtomStart + RelativeOffset)[0] == Stamp for RelativeOffset in Relatives))
    assert BytesA == 258
...
def TestFVPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/configuration/fillet/views/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestLPAMITAC() -> None:
    SingleData = EncodeConfigZeroStream(part_name='Part1', atoms=((102, 40), (101, 32)), high_water=(102, 105))
    PatternData = EncodeConfigZeroStream(part_name='Part1', atoms=((102, 40), (101, 32)), high_water=(102, 105), annotation_view_count=2, annotation_view_variant='linear_pattern')
    AnnotationTag = EncodeClassDefinition('moAnnotationView_c', 1)
...
def TestCPAMMNPM() -> None:
    LinearData = EncodeConfigZeroStream(part_name='Part1', atoms=((102, 40), (101, 32)), high_water=(102, 105), annotation_view_count=2, annotation_view_variant='linear_pattern')
    CircularData = EncodeConfigZeroStream(part_name='Part1', atoms=((102, 46), (101, 32)), high_water=(102, 105), annotation_view_count=2, annotation_view_variant='circular_pattern')
    ExpectedData = EncodeConfigZeroStream(part_name='Part1', atoms=((102, 46), (101, 32)), high_water=(102, 105), annotation_view_count=2, annotation_view_variant='linear_pattern')
    assert CircularData == ExpectedData
    assert len(CircularData) == len(LinearData)

# keeps this focused behavior isolated so regressions remain immediately visible
def TestPVPCNEVB() -> None:
    ProgramPath = FilePath(__file__).parents[4] / 'src/convert/adapters/solidworks/programs/configuration/pattern/views/Program.py'
    SourceText = ProgramPath.read_text(encoding='utf-8')
    assert 'bytes.fromhex' not in SourceText
    assert 'base64' not in SourceText
    assert 'b85decode' not in SourceText
...
def TestEFFAOMA(FeatureCount: int) -> None:
    ...
    assert StructLib.unpack_from('<II', StreamData, AtomPos - 8) == (100 + FeatureCount, FeatureCount)
...
def TestAICOTIF() -> None:
    ...
    Changed = [Index for Index, (FirstByte, SecondByte) in enumerate(zip(FirstData, SecondData)) if FirstByte != SecondByte]
    assert Changed
...
# keeps this focused behavior isolated so regressions remain immediately visible
@PytestLib.mark.parametrize('AtomData', (((-1, 32),), ((101, -1),), ((4294967296, 32),)))
