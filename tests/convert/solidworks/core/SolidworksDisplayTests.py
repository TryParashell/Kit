...
# keeps this focused behavior isolated so regressions remain immediately visible
@PytestLib.mark.parametrize('ItemValue', ('Top Plane@Rotor@Assembly', 'Belt1-1^Assembly-1@Assembly', 'C:/Parts/Rotor.SLDPRT', '@Assembly'))
...
def TestDGISTMAMTS() -> None:
    ...
    assert FaceInfo.positions_mm[0] == PytestLib.approx((-13.599040918052197, 7.011139299720526, -0.9284935076721013))
    assert FaceInfo.normals[0] == PytestLib.approx((0.8888262510299683, -0.4582444131374359, 0.0))
    assert max((Index for Triangle in FaceInfo.triangle_indices for Index in Triangle)) < len(FaceInfo.positions_mm)
