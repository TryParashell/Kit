...
def TestSLNPSAD() -> None:
    ...
    assert ModelStamps == (int(KeywordRoot.attrib['id']), int(NativeModel.attrib['swLastModifiedStamp']), 101)
...
# keeps this focused behavior isolated so regressions remain immediately visible
@PytestLib.mark.skipif(not KFreeCadRevPin.is_file(), reason='top plane pin revolution corpus unavailable')
