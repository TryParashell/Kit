...
from convert.adapters.solidworks.core.Topology import BOSS_OPERATION as Operation, CUT_OPERATION as OperationA, FeatureTopology, FULL_REVOLUTION_END as EndInfo, REVOLVE_BOSS_OPERATION as OperationB, REVOLVE_CUT_OPERATION as OperationC, REVOLVE_SUPPORTS as Supports, SUPPORTED_END_CONDITIONS as ConditionsA

# centralizes shared evidence so every related assertion uses one value
KRootInfo = FilePath(__file__).resolve().parents[3] / 'fixtures' / 'solidworks' / 'donors'
...
def TestTMLETCFD() -> None:
    ...
    OnDisk = sorted((TargetPath.name for TargetPath in KRootInfo.iterdir() if TargetPath.is_dir()))
...
def TestTMTTCFB() -> None:
    Donors = ManifestDonors()
    ResolvedBytes = sum((len((KRootInfo / DonorId / KNameInfoA).read_bytes()) for DonorId in Donors))
    ContainerBytes = 0
...
def TestECFMIMD() -> None:
    for DonorId, RecordInfo in ManifestDonors().items():
        ...
        for NameText, Entry in Index.items():
            ...
            assert Entry['sha256'] == Hashlib.sha256(Payload).hexdigest(), (DonorId, NameText)
            assert Entry['file'] == f'{KDirectory}/{TargetPath.name}', (DonorId, NameText)
...
def TestEMFAWTMAIS() -> None:
    for DonorId, RecordInfo in ManifestDonors().items():
        ...
        Streams = {ItemValue['name']: ItemValue['file'] for ItemValue in MetaInfoA['container_streams']}
...
def TestEMFDOEPF() -> None:
    for DonorId in ManifestDonors():
        ...
        for LookupKey in ('axis_directions', 'swept_arc_counts', 'inherited_directions'):
            assert len(MetaInfoA[LookupKey]) <= Count, (DonorId, LookupKey)
        assert len(MetaInfoA['spare_plane_names']) == len(MetaInfoA['spare_plane_ids']), DonorId
        assert len(MetaInfoA['spare_plane_frames']) == len(MetaInfoA['spare_plane_ids']), DonorId
        assert len(set(MetaInfoA['spare_equations'])) == len(MetaInfoA['spare_equations']), DonorId
