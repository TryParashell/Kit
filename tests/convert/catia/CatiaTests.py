...
from convert.adapters.catia import CatiaAdapter, CatiaAdapterError, Cfv2Archive as CfvTwoArchive, Cfv2FormatError as CfvTwoFormatError, OsmxArchive, OsmxFormatError, append_cfv2_stream as AppendCfvTwoStream, build_cfv2 as BuildCfvTwo, build_declaration as BuildDecl, read_catia as ReadCatia, write_catia as WriteCatia
...
from convert.adapters.solidworks import read_sldprt as ReadSldprt, write_sldprt as WriteSldprt
from convert.geometry.Parasolid import encode_brep_model as EncodeBrepModel
from interchange import BrepPayload, Capability, Configuration as Config, Diagnostic as DiagValue, NativeFeatureDefinition, PayloadRole, Provenance, Severity, frozen_mapping as FrozenMapping
...
def TestFormatNames() -> None:
    ...
    assert SuffixByDocType == {DocType: Suffix for Suffix, DocType in DocTypeBySuffix.items()}

# this definition exists because focused behavior needs one stable owner
def PackedManifest(RawValue: bytes) -> bytes:
    return b''.join((b'KITCFV2\x01', Struct.pack('>Q', len(RawValue)), Hashlib.sha256(RawValue).digest(), ZlibValue.compress(RawValue)))

# this definition exists because focused behavior needs one stable owner
def Parasolid(PayloadId: str, DataValue: bytes, *, KindValue: str='partition', FormatId: str='parasolid') -> BrepPayload:
    return BrepPayload(PayloadId, FormatId, KindValue, 'SCH_SW_32001_11000', Hashlib.sha256(DataValue).hexdigest(), data=DataValue, source_stream=PayloadId, role=PayloadRole.BREP, file_extension='.x_b' if FormatId != 'catia.cgm' else '.cgm')

# this definition exists because focused behavior needs one stable owner
def Opencascade(PayloadId: str, DataValue: bytes, *, KindValue: str='shape', FormatId: str='opencascade') -> BrepPayload:
    return BrepPayload(PayloadId, FormatId, KindValue, 'CASCADE Topology V1', Hashlib.sha256(DataValue).hexdigest(), data=DataValue, source_stream=f'{PayloadId}.brep', role=PayloadRole.BREP, file_extension='.brep')
...
def TestCarrierOne() -> None:
    ...
    assert next((ItemValue for ItemValue in Restored.brep_payloads if ItemValue.id == Payload.id)) == Payload

# this definition exists because focused behavior needs one stable owner
def TestCarrierOrA() -> None:
    Encoded = EncodeBrepModel(TriangleBrep())
    for Payloads in ((Parasolid('partition:1', Encoded), Parasolid('partition:2', Encoded)), (Parasolid('partition', Encoded), Parasolid('delta', Encoded, KindValue='deltas'))):
        Output = BytesIo()
        WriteCatia(Replace(DocValue(), brep_payloads=Payloads), Output, allow_non_native=True)
        Restored = ReadCatia(Output.getvalue())
        assert Restored.brep is None
        assert tuple((ItemValue for ItemValue in Restored.brep_payloads if ItemValue.id in {PointValue.id for PointValue in Payloads})) == Payloads

# this definition exists because focused behavior needs one stable owner
def TestCgmPayload() -> None:
    Encoded = EncodeBrepModel(TriangleBrep())
    Payload = Parasolid('catia:native-cgm', Encoded, KindValue='native_brep', FormatId='catia.cgm')
    Output = BytesIo()
    WriteCatia(Replace(DocValue(), brep_payloads=(Payload,)), Output, allow_non_native=True)
    Restored = ReadCatia(Output.getvalue())
    assert Restored.brep is None
    assert next((ItemValue for ItemValue in Restored.brep_payloads if ItemValue.id == Payload.id)) == Payload

# this definition exists because focused behavior needs one stable owner
@Pytest.mark.parametrize('FormatId', ('freecad.brep', 'opencascade', 'opencascade.brep'))
...
def TestCarrierOr() -> None:
    Encoded = BrepModelBrep(TriangleBrep())
    for Payloads in ((Opencascade('shape:1', Encoded), Opencascade('shape:2', Encoded)), (Opencascade('shape', Encoded), Opencascade('delta', Encoded, KindValue='delta')), (Opencascade('shape', Encoded), Parasolid('partition', EncodeBrepModel(TriangleBrep())))):
        Output = BytesIo()
        WriteCatia(Replace(DocValue(), brep_payloads=Payloads), Output, allow_non_native=True)
        Restored = ReadCatia(Output.getvalue())
        assert Restored.brep is None
...
def TestCfvTwoEvery() -> None:
    ...
    Generated = CfvTwoArchive.from_bytes(AppendCfvTwoStream(Source, 'KitInterchange', b'manifest'))
...
def TestRoundtripIs(Source: Path, TempPath: Path) -> None:
    ...
    Result = Registry.write(DocValue, Output, options=WriteOptions(values={'portable': False, 'allow_carrier': DocValue.assembly is not None, 'require_self_contained': DocValue.assembly is None}))
    assert Result.metadata['mode'] == 'exact_native_roundtrip'
    ...
    assert Result.requirements == (('referenced CATIA component files',) if DocValue.assembly is not None else ())
...
# this definition exists because focused behavior needs one stable owner
@Pytest.mark.parametrize(('source', 'write_values'), ((KCatparts / 'Banjo.CATPart', {'rebuild': True}), (KCatproducts / 'Tilton_Set.CATProduct', {})))
def TestGeneratedC(Source: Path, WriteValues: dict[str, bool], TempPath: Path) -> None:
    ...
    SavedDoc = next((Payload for Payload in Restored.brep_payloads if Payload.id.startswith('catia:preserved-native-document:')))
    Token = SavedDoc.id.removeprefix('catia:preserved-native-document:')
    SavedBinding = next((Payload for Payload in Restored.brep_payloads if Payload.id == f'catia:preserved-native-document-binding:{Token}'))
...
def TestCatpartGeom() -> None:
    ...
    assert [Payload.schema for Payload in NativeContainers] == [DeclValue.class_name for DeclValue in Archive.declarations()]
    assert [Payload.source_stream for Payload in NativeContainers] == [DeclValue.stream_name for DeclValue in Archive.declarations()]
    CgmDecl = next((ItemValue for ItemValue in Archive.declarations() if ItemValue.class_name == 'CGMGeom'))
    CgmStream = Archive.outer.stream(CgmDecl.stream_name)
    assert CgmStream is not None
    ...
    CgmMeta = next((ItemValue for ItemValue in DocValue.metadata['catia.container_declarations'] if ItemValue['class_name'] == 'CGMGeom'))
    assert CgmMeta['sha256'] == CgmValue.sha256
    assert CgmMeta['logical_length'] == len(CgmValue.data or b'')
    FeatureGraph = next((Payload for Payload in DocValue.brep_payloads if Payload.id == 'catia:native-feature-graph'))
    assert FeatureGraph.kind == 'native_feature_graph'
    assert OsmxArchive.from_bytes(FeatureGraph.data or b'').version == 'V5R28SP6HF0'
    assert [Plane.name for Plane in DocValue.support_planes] == ['xy-plane', 'yz-plane', 'zx-plane']
    assert DocValue.bodies[0].name == 'Body.2'
    assert DocValue.feature_timeline[0].attributes['native_payload_id'] == FeatureGraph.id
    ...
    assert DocValue.capabilities == frozenset({Capability.PARAMETRIC_HISTORY, Capability.SUPPORT_PLANES, Capability.BODY_STRUCTURE, Capability.CONFIGURATIONS, Capability.BREP, Capability.TESSELLATION, Capability.NATIVE_PAYLOADS, Capability.PROVENANCE, Capability.ROUNDTRIP_METADATA})
    WithoutData = CatiaAdapter().read(Source, ReadOptions(include_brep=False, include_tessellation=False))
    NativeDoc = next((Payload for Payload in WithoutData.brep_payloads if Payload.kind == 'native_document'))
    assert NativeDoc.data is None
    assert {Payload.kind for Payload in WithoutData.brep_payloads if Payload.kind != 'native_document'} == {'native_document_binding', 'native_feature_graph', 'native_product_graph', 'native_brep', 'brep_topology', 'brep_mode', 'native_tessellation', 'native_container'}
    assert all((Payload.data is None for Payload in WithoutData.brep_payloads if Payload.role in {PayloadRole.BREP, PayloadRole.TESSELLATION, PayloadRole.DOCUMENT}))
    assert all((Payload.data is not None for Payload in WithoutData.brep_payloads if Payload.role in {PayloadRole.FEATURE_HISTORY, PayloadRole.ASSEMBLY_STRUCTURE, PayloadRole.AUXILIARY}))
...
def TestCatpart() -> None:
    ...
    Product = next((Value for Value in Source.declarations() if Value.class_name == 'CATProdCont'))
    PartValue = next((Value for Value in Source.declarations() if Value.class_name == 'CATPrtCont'))
    ...
    Declarations = b''.join((BuildDecl(Product.class_name, Product.base_class, ProductStreamName, 1), BuildDecl(PartValue.class_name, PartValue.base_class, PartStreamName, 2), BuildDecl('CompanyFeatureCont', 'CATFeatCont', CustomStreamName, 3)))
    Generated = BuildCfvTwo((('Format', b'CATPart'), ('Data', Declarations), (ProductStreamName, Source.stream_bytes(ProductStream)), (PartStreamName, Source.stream_bytes(PartStream)), (CustomStreamName, CustomData)))
    DocValue = CatiaAdapter().read(Generated, ReadOptions(include_brep=False))
    Custom = next((Payload for Payload in DocValue.brep_payloads if Payload.schema == 'CompanyFeatureCont'))
    ...
    Changed = Replace(Custom, data=ChangedData, sha256=Hashlib.sha256(ChangedData).hexdigest())
    Modified = Replace(DocValue, brep_payloads=tuple((Changed if Payload.id == Custom.id else Payload for Payload in DocValue.brep_payloads)))
    Output = BytesIo()
...
def TestCatpartRoot() -> None:
    ...
    ProductStreamName = '1000_00000001_1'
    PartStreamName = '1000_00000002_2'
    Declarations = b''.join((BuildDecl('CompanyProductRoot', 'CATFeatCont', ProductStreamName, 1), BuildDecl('CompanyPartRoot', 'CompanyProductRoot', PartStreamName, 2)))
    Generated = BuildCfvTwo((('Format', b'CATPart'), ('Data', Declarations), (ProductStreamName, Source.stream_bytes(ProductStream)), (PartStreamName, Source.stream_bytes(PartStream))))
    ...
    assert tuple((Stream['class_name'] for Stream in DocValue.metadata['catia.osmx_streams'])) == ('CompanyProductRoot', 'CompanyPartRoot')
...
def TestCatpartFrom() -> None:
    ...
    Product = next((Value for Value in Source.declarations() if Value.class_name == 'CATProdCont'))
    PartValue = next((Value for Value in Source.declarations() if Value.class_name == 'CATPrtCont'))
    CgmValue = next((Value for Value in Source.declarations() if Value.class_name == 'CGMGeom'))
    ProductStream = Source.outer.stream(Product.stream_name)
    PartStream = Source.outer.stream(PartValue.stream_name)
    ...
    Declarations = b''.join((BuildDecl(Product.class_name, Product.base_class, ProductStreamName, 1), BuildDecl(PartValue.class_name, PartValue.base_class, PartStreamName, 2), BuildDecl('CompanyGeometryContainer', 'CATContainer', RenamedCgmStreamName, 3), BuildDecl('CGMGeom', 'CATContainer', MisleadingStreamName, 4)))
    Generated = BuildCfvTwo((('Format', b'CATPart'), ('Data', Declarations), (ProductStreamName, Source.stream_bytes(ProductStream)), (PartStreamName, Source.stream_bytes(PartStream)), (RenamedCgmStreamName, Source.stream_bytes(CgmStream)), (MisleadingStreamName, b'opaque-company-payload')))
    DocValue = CatiaAdapter().read(Generated, ReadOptions(include_brep=False))
    RenamedCgm = next((Payload for Payload in DocValue.brep_payloads if Payload.schema == 'CompanyGeometryContainer'))
    Misleading = next((Payload for Payload in DocValue.brep_payloads if Payload.schema == 'CGMGeom'))
...
def TestCatpartC() -> None:
    ...
    Struct.pack_into('<I', PartGraph, SymbolTableOffset + 2, len(PartGraph) - SymbolTableOffset)
    ProductStreamName = '1000_00000001_1'
    PartStreamName = '1000_00000002_2'
    Declarations = b''.join((BuildDecl(Product.class_name, Product.base_class, ProductStreamName, 1), BuildDecl(PartValue.class_name, PartValue.base_class, PartStreamName, 2)))
    Generated = BuildCfvTwo((('Format', b'CATPart'), ('Data', Declarations), (ProductStreamName, Source.stream_bytes(ProductStream)), (PartStreamName, bytes(PartGraph))))
    DocValue = CatiaAdapter().read(Generated, ReadOptions(include_brep=False))
    assert FeatureType.decode('ascii') in DocValue.metadata['catia.native_symbols']
    assert DocValue.feature_timeline[0].attributes['native_symbols'] == DocValue.metadata['catia.native_symbols']
    Definition = DocValue.feature_timeline[0].definition
    assert isinstance(Definition, NativeFeatureDefinition)
...
def TestCatpartAre() -> None:
    ...
    for SymbolIndex, NameValue in zip(Indices, Names, strict=True):
        ...
        PartData[Symbol.offset:Symbol.offset + len(NameValue)] = NameValue.encode('ascii')
    ProductStreamName = '1000_00000001_1'
    PartStreamName = '1000_00000002_2'
    Declarations = b''.join((BuildDecl(Product.class_name, Product.base_class, ProductStreamName, 1), BuildDecl(PartValue.class_name, PartValue.base_class, PartStreamName, 2)))
    Generated = BuildCfvTwo((('Format', b'CATPart'), ('Data', Declarations), (ProductStreamName, Source.stream_bytes(ProductStream)), (PartStreamName, bytes(PartData))))
    DocValue = CatiaAdapter().read(Generated)
    assert [Plane.name for Plane in DocValue.support_planes] == list(Names)
    assert [Plane.attributes['principal_index'] for Plane in DocValue.support_planes] == [0, 1, 2]
...
def TestCatproduct() -> None:
    ...
    assert [Instance.name for Instance in DocValue.assembly.instances] == ['I_4876.2', 'I_4876.3', 'I_4784.5', 'I_Brake_bias_90_degree_coupler.1']
    assert len(DocValue.assembly.definitions) == 5
    assert [Payload.schema for Payload in DocValue.brep_payloads[2:]] == [DeclValue.class_name for DeclValue in Archive.declarations()]
    assert DocValue.capabilities == frozenset({Capability.ASSEMBLIES, Capability.PARAMETRIC_HISTORY, Capability.SUPPORT_PLANES, Capability.BODY_STRUCTURE, Capability.COMPONENT_DOCUMENTS, Capability.CONFIGURATIONS, Capability.EXTERNAL_REFERENCES, Capability.BREP, Capability.TESSELLATION, Capability.NATIVE_PAYLOADS, Capability.PROVENANCE, Capability.ROUNDTRIP_METADATA})
...
def TestPedalBody() -> None:
    ...
    assert NativeSymbols == tuple(dict.fromkeys((Value for Value in DocValue.metadata['catia.part_symbols'] if Value)))
    assert {'GSMPlane', 'GSMPoint', 'GSMPointCoord', 'GSMAxisToAxis', 'GSMTranslate', 'AxisSystem', 'SectioningPlane'} <= set(NativeSymbols)
...
def TestCatpartCgr() -> None:
    Source = KCatparts / 'Banjo.CATPart'
    Archive = CfvTwoArchive.from_bytes(Source.read_bytes())
    DocValue = CatiaAdapter().read(Source, ReadOptions(include_brep=False, include_tessellation=True))
    Payload = next((ItemValue for ItemValue in DocValue.brep_payloads if ItemValue.format_id == 'catia.cgr'))
    DeclValue = next((ItemValue for ItemValue in Archive.declarations() if ItemValue.class_name == 'CATCGRCont'))
...
def TestSolidworksA(TempPath: Path) -> None:
    ...
    assert [Value.class_name for Value in Archive.declarations()] == ['CATProdCont', 'CATPrtCont']
    assert any((Folder.stream('KitInterchange') is not None for Folder in Archive.nested))
    Restored = OpenDoc(Output)
    assert Restored.source.format_id == 'catia.v5'
    assert Restored.metadata['catia.embedded_source_format_id'] == Source.source.format_id
    ...
    Retained = tuple((Payload for Payload in Restored.brep_payloads if Payload.kind not in {'native_document', 'native_document_binding'}))
    assert Retained == Source.brep_payloads
    assert sum((Payload.kind == 'native_document_binding' for Payload in Restored.brep_payloads)) == 1
...
# this definition exists because focused behavior needs one stable owner
@Pytest.mark.parametrize(('source', 'wrong_suffix'), ((KCatparts / 'Banjo.CATPart', '.CATProduct'), (KCatproducts / 'Tilton_Set.CATProduct', '.CATPart')))
...
# this definition exists because focused behavior needs one stable owner
@Pytest.mark.parametrize(('marker', 'wrong_suffix'), ((b'CATPart', '.CATProduct'), (b'CATProduct', '.CATPart')))
...
def TestReaderPart() -> None:
    ProductStreamName = '1000_00000001_1'
    PartStreamName = '1000_00000002_2'
    Declarations = b''.join((BuildDecl('CATProdCont', 'CATFeatCont', ProductStreamName, 1), BuildDecl('CATPrtCont', 'CATFeatCont', PartStreamName, 2)))
    DataValue = BuildCfvTwo((('Format', b'CATPart'), ('Data', Declarations), (ProductStreamName, b'product-root'), (PartStreamName, b'part-root')))
...
def TestModifiedDoc(TempPath: Path) -> None:
    ...
    assert tuple(((Stream.name, OutputArchive.stream_bytes(Stream, OutputArchive.outer)) for Stream in OutputArchive.outer.streams if Stream.name != 'KitInterchange')) == tuple(((Stream.name, OriginalArchive.stream_bytes(Stream, OriginalArchive.outer)) for Stream in OriginalArchive.outer.streams))
    Restored = OpenDoc(Output)
    assert Restored.source.format_id == 'catia.v5'
    ...
    Retained = tuple((Payload for Payload in Changed.brep_payloads if Payload.kind not in {'native_document', 'native_document_binding'}))
    RestoredRetained = tuple((Payload for Payload in Restored.brep_payloads if Payload.kind not in {'native_document', 'native_document_binding'}))
    assert RestoredRetained == Retained
    assert sum((Payload.kind == 'native_document_binding' for Payload in Restored.brep_payloads)) == 2
...
def TestEmbeddedAnd(TempPath: Path) -> None:
    ...
    assert all((Payload.data is None for Payload in Filtered.brep_payloads if Payload.role in {PayloadRole.BREP, PayloadRole.DOCUMENT}))
    Binding = next((Payload for Payload in Filtered.brep_payloads if Payload.kind == 'native_document_binding'))
...
def TestRecomputed() -> None:
    ...
    Changed = Replace(DocValue, feature_timeline=(Replace(Feature, name='Forged CATIA feature'),))
    Changed = Replace(Changed, metadata=FrozenMapping({**Changed.metadata, 'catia.roundtrip_sha256': SemanticDigest(Changed)}))
    Output = BytesIo()
    Result = WriteCatia(Changed, Output, allow_non_native=True)
    assert Result.metadata['mode'] == 'native_base_with_neutral_edits'
    assert Result.application_usable is False
    assert Result.vendor_loadable is False
    assert ReadCatia(Output.getvalue()).feature_timeline[0].name == 'Forged CATIA feature'

# this definition exists because focused behavior needs one stable owner
def TestSwappedDoc() -> None:
    DocValue = OpenDoc(KCatparts / 'Banjo.CATPart')
    Replacement = (KCatparts / 'Bolt_M5x40.CATPart').read_bytes()
    Changed = Replace(DocValue, brep_payloads=tuple((Replace(Payload, data=Replacement, sha256=Hashlib.sha256(Replacement).hexdigest()) if Payload.kind == 'native_document' else Payload for Payload in DocValue.brep_payloads)))
    Output = BytesIo()
    Result = WriteCatia(Changed, Output, allow_non_native=True)
    assert Result.metadata['mode'] == 'generated_cfv2'
...
def TestMutated() -> None:
    ...
    Changed = Replace(DocValue, brep_payloads=tuple((Replace(Payload, data=Native, sha256=Hashlib.sha256(Native).hexdigest()) if Payload.kind == 'native_document' else Payload for Payload in DocValue.brep_payloads)))
    Output = BytesIo()
    Result = WriteCatia(Changed, Output, allow_non_native=True)
    assert Result.metadata['mode'] == 'generated_cfv2'
...
def TestGenerated() -> None:
    ...
    Changed = Replace(DocValue, brep_payloads=tuple((Replace(Payload, data=Native, sha256=Hashlib.sha256(Native).hexdigest()) if Payload.kind == 'native_document' else Replace(Payload, data=NativeDigest, sha256=Hashlib.sha256(NativeDigest).hexdigest()) if Payload.kind == 'native_document_binding' else Payload for Payload in DocValue.brep_payloads)))
    Rebuilt = BytesIo()
    Result = WriteCatia(Changed, Rebuilt, allow_non_native=True)
    assert Result.metadata['mode'] == 'generated_cfv2'
    assert Rebuilt.getvalue() != Native
...
def TestGeneratedD(Change: str) -> None:
    ...
    if Change == 'capabilities':
        Changed = Replace(DocValue, capabilities=DocValue.capabilities | {Capability.MATERIALS})
    elif Change == 'metadata':
        Changed = Replace(DocValue, metadata=FrozenMapping({**DocValue.metadata, 'user.tag': 'changed'}))
    else:
        Changed = Replace(DocValue, diagnostics=(*DocValue.diagnostics, DiagValue('user.changed', 'changed', Severity.INFO)))
    Rebuilt = BytesIo()
    Result = WriteCatia(Changed, Rebuilt, allow_non_native=True)
    assert Result.metadata['mode'] == 'generated_cfv2'
...
def TestForeignRole(RoleValue: PayloadRole) -> None:
    ...
    Foreign = BrepPayload(f'foreign:{RoleValue.value}', 'future.cad', 'foreign_payload', '1', Hashlib.sha256(PayloadData).hexdigest(), data=PayloadData, role=RoleValue, file_extension='.bin')
    ...
    Changed = Replace(DocValue, brep_payloads=tuple((Replace(Payload, data=ChangedData, sha256=Hashlib.sha256(ChangedData).hexdigest()) if Payload.id == Foreign.id else Payload for Payload in DocValue.brep_payloads)))
    Rebuilt = BytesIo()
    Result = WriteCatia(Changed, Rebuilt, allow_non_native=True)
    assert Result.metadata['mode'] == 'generated_cfv2'
...
def TestLegacyDocIs() -> None:
    ...
    Legacy = BrepPayload('catia:native-document-binding', 'catia.v5.sha256', 'native_document_binding', 'sha256', Hashlib.sha256(LegacyData).hexdigest(), data=LegacyData, role=PayloadRole.DOCUMENT, file_extension='.sha256')
    Carried = Replace(Source, brep_payloads=(*Source.brep_payloads, Legacy))
    Carrier = BytesIo()
    WriteCatia(Carried, Carrier, allow_non_native=True)
    Restored = CatiaAdapter().read(Carrier.getvalue())
    Bindings = tuple((Payload for Payload in Restored.brep_payloads if Payload.kind == 'native_document_binding'))
...
def TestForeign() -> None:
    Source = OpenDoc(KSldprt)
    PayloadData = b'foreign-payload'
    Foreign = BrepPayload('foreign:auxiliary', 'future.cad', 'foreign_payload', '1', Hashlib.sha256(PayloadData).hexdigest(), data=PayloadData, role=PayloadRole.AUXILIARY, file_extension='.bin')
    Carried = Replace(Source, brep_payloads=(*Source.brep_payloads, Foreign))
    Carrier = BytesIo()
    WriteCatia(Carried, Carrier, allow_non_native=True)
    DocValue = CatiaAdapter().read(Carrier.getvalue())
    Changed = Replace(DocValue, brep_payloads=tuple((Replace(Payload, source_stream='changed', provenance=Provenance('future.cad', 'changed'), attributes=FrozenMapping({'user.tag': 'changed'})) if Payload.id == Foreign.id else Payload for Payload in DocValue.brep_payloads)))
    Rebuilt = BytesIo()
    Result = WriteCatia(Changed, Rebuilt, allow_non_native=True)
    assert Result.metadata['mode'] == 'generated_cfv2'
    Restored = CatiaAdapter().read(Rebuilt.getvalue())
    Payload = next((ItemValue for ItemValue in Restored.brep_payloads if ItemValue.id == Foreign.id))
...
def TestCfvTwo() -> None:
    ...
    assert Stream is not None
    Struct.pack_into('>I', DataValue, Stream.descriptor_offset + 84, Archive.outer.offset)
...
def TestCfvTwoA() -> None:
    ...
    Struct.pack_into('>I', DataValue, Second.descriptor_offset + 84, First.extents[0].physical_offset)
...
def TestCfvTwoMagic() -> None:
    ...
    assert Start + len(Injected) < Preview.extents[0].physical_offset + Preview.logical_length
    DataValue[Start:Start + len(Injected)] = Injected
...
# this definition exists because focused behavior needs one stable owner
@Pytest.mark.parametrize(('manifest', 'message'), ((b'KITCFV2\x01' + Struct.pack('>Q', 1 << 63) + bytes(32) + ZlibValue.compress(b''), 'size limit'), (b'KITCFV2\x01' + Struct.pack('>Q', 1) + Hashlib.sha256(b'ab').digest() + ZlibValue.compress(b'ab'), 'declared length'), (b'KITCFV2\x01' + Struct.pack('>Q', 2) + Hashlib.sha256(b'{}').digest() + ZlibValue.compress(b'{}') + b'trailing', 'trailing compressed data')))
...
def TestDuplicate() -> None:
    Nested = BuildCfvTwo((('KitInterchange', b'short'), ('OtherManifestX', b'a different and longer manifest')))
    ...
    assert sum((Stream.name == 'KitInterchange' for Stream in Reproduced.nested[0].streams)) == 2
...
# this binding exists because shared behavior needs one stable value
globals()['test_adapter_capabilities_cover_the_complete_interchange_schema'] = TestAdapterThe

# this binding exists because shared behavior needs one stable value
globals()['test_catia_carrier_decodes_one_parasolid_model_without_changing_payloads'] = TestCarrierOne

# this binding exists because shared behavior needs one stable value
globals()['test_catia_carrier_decodes_supported_opencascade_payloads'] = TestCarrier

# this binding exists because shared behavior needs one stable value
globals()['test_catia_carrier_refuses_ambiguous_or_delta_opencascade_models'] = TestCarrierOr

# this binding exists because shared behavior needs one stable value
globals()['test_catia_carrier_refuses_ambiguous_or_delta_parasolid_models'] = TestCarrierOrA

# this binding exists because shared behavior needs one stable value
globals()['test_catia_cgm_payload_is_never_treated_as_parasolid'] = TestCgmPayload

# this binding exists because shared behavior needs one stable value
globals()['test_catia_destinations_enforce_document_kind'] = TestEnforceDoc

# this binding exists because shared behavior needs one stable value
globals()['test_catia_format_names_have_one_authoritative_bijection'] = TestFormatNames

# this binding exists because shared behavior needs one stable value
globals()['test_catia_reader_rejects_carrier_suffix_kind_mismatch'] = TestReaderKind

# this binding exists because shared behavior needs one stable value
globals()['test_catia_reader_rejects_contradictory_part_and_product_roots'] = TestReaderPart

# this binding exists because shared behavior needs one stable value
globals()['test_catia_reader_rejects_native_suffix_kind_mismatch'] = TestReaderKindA

# this binding exists because shared behavior needs one stable value
globals()['test_catia_reader_uses_content_before_suffix_without_declarations'] = TestReaderUses

# this binding exists because shared behavior needs one stable value
globals()['test_cfv2_ignores_nested_magic_away_from_stream_boundaries'] = TestCfvTwoMagic

# this binding exists because shared behavior needs one stable value
globals()['test_cfv2_native_stream_append_preserves_every_source_stream'] = TestCfvTwoEvery

# this binding exists because shared behavior needs one stable value
globals()['test_cfv2_rejects_extent_inside_directory'] = TestCfvTwo

# this binding exists because shared behavior needs one stable value
globals()['test_cfv2_rejects_inconsistent_outer_directory'] = TestCfvTwoOuter

# this binding exists because shared behavior needs one stable value
globals()['test_cfv2_rejects_overlapping_extents'] = TestCfvTwoA

# this binding exists because shared behavior needs one stable value
globals()['test_cfv2_rejects_unowned_nested_container'] = TestCfvTwoB

# this binding exists because shared behavior needs one stable value
globals()['test_changed_cgm_bytes_disable_exact_native_replay'] = TestChangedCgm

# this binding exists because shared behavior needs one stable value
globals()['test_conversion_result_reports_selected_catia_reader'] = TestConversion

# this binding exists because shared behavior needs one stable value
globals()['test_deep_manifest_json_is_rejected_without_recursion_leak'] = TestDeepIsLeak

# this binding exists because shared behavior needs one stable value
globals()['test_duplicate_nested_manifest_descriptors_are_rejected'] = TestDuplicate

# this binding exists because shared behavior needs one stable value
globals()['test_embedded_manifest_applies_read_options_and_replays_exactly'] = TestEmbeddedAnd

# this binding exists because shared behavior needs one stable value
globals()['test_embedded_manifest_preserves_foreign_document_and_auxiliary_payloads'] = TestEmbeddedDoc

# this binding exists because shared behavior needs one stable value
globals()['test_embedded_manifest_rejects_unknown_configuration'] = TestEmbedded

# this binding exists because shared behavior needs one stable value
globals()['test_engine_reports_solidworks_alias_and_output_adapter'] = TestEngineAlias

# this binding exists because shared behavior needs one stable value
globals()['test_foreign_envelope_role_payload_mutation_invalidates_replay'] = TestForeignRole

# this binding exists because shared behavior needs one stable value
globals()['test_foreign_payload_fields_disable_exact_replay'] = TestForeign

# this binding exists because shared behavior needs one stable value
globals()['test_generated_carrier_binding_rejects_coordinated_mutation'] = TestGenerated

# this binding exists because shared behavior needs one stable value
globals()['test_generated_carrier_binding_rejects_mutated_physical_stream'] = TestGeneratedA

# this binding exists because shared behavior needs one stable value
globals()['test_generated_carrier_preserves_declared_sparse_capabilities'] = TestGeneratedB

# this binding exists because shared behavior needs one stable value
globals()['test_generated_carrier_preserves_native_pair_for_exact_replay'] = TestGeneratedC

# this binding exists because shared behavior needs one stable value
globals()['test_generated_carrier_semantic_edits_disable_exact_replay'] = TestGeneratedD

# this binding exists because shared behavior needs one stable value
globals()['test_legacy_document_binding_is_normalized_once'] = TestLegacyDocIs

# this binding exists because shared behavior needs one stable value
globals()['test_manifest_decompression_is_bounded'] = TestManifestIs

# this binding exists because shared behavior needs one stable value
globals()['test_modified_native_document_preserves_native_base_with_neutral_edits'] = TestModifiedDoc

# this binding exists because shared behavior needs one stable value
globals()['test_mutated_native_catproduct_cannot_exact_replay'] = TestMutated

# this binding exists because shared behavior needs one stable value
globals()['test_native_catia_roundtrip_is_byte_exact'] = TestRoundtripIs

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_classifies_container_roles_from_payload_structure'] = TestCatpartFrom

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_discovers_custom_root_classes_structurally'] = TestCatpartRoot

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_exposes_unrecognized_declared_containers'] = TestCatpart

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_principal_plane_names_are_discovered_structurally'] = TestCatpartAre

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_rejects_unknown_configuration'] = TestCatpartA

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_replays_across_solidworks_carrier'] = TestCatpartB

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_retains_declared_cgr_tessellation_on_request'] = TestCatpartCgr

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_retains_declared_geometry_and_feature_graphs'] = TestCatpartGeom

# this binding exists because shared behavior needs one stable value
globals()['test_native_catpart_symbol_discovery_preserves_customer_feature_types'] = TestCatpartC

# this binding exists because shared behavior needs one stable value
globals()['test_native_catproduct_retains_product_occurrences'] = TestCatproduct

# this binding exists because shared behavior needs one stable value
globals()['test_osmx_rejects_excessive_symbol_count'] = TestOsmxRejects

# this binding exists because shared behavior needs one stable value
globals()['test_pedal_body_exposes_native_parametric_symbols'] = TestPedalBody

# this binding exists because shared behavior needs one stable value
globals()['test_pre_payload_field_catpart_manifest_restores_roles_and_envelope'] = TestPrePayload

# this binding exists because shared behavior needs one stable value
globals()['test_public_sdk_defaults_to_portable_catproduct_writes'] = TestPublicSdkTo

# this binding exists because shared behavior needs one stable value
globals()['test_real_catia_corpus_uses_valid_cfv2_directories'] = TestRealCorpus

# this binding exists because shared behavior needs one stable value
globals()['test_recomputed_roundtrip_digest_cannot_forge_native_catpart_semantics'] = TestRecomputed

# this binding exists because shared behavior needs one stable value
globals()['test_shallow_malformed_manifest_json_is_rejected_by_probe_and_read'] = TestShallowIsBy

# this binding exists because shared behavior needs one stable value
globals()['test_solidworks_assembly_roundtrips_through_generated_catproduct'] = TestSolidworks

# this binding exists because shared behavior needs one stable value
globals()['test_solidworks_part_roundtrips_through_generated_catpart'] = TestSolidworksA

# this binding exists because shared behavior needs one stable value
globals()['test_stripped_carrier_metadata_cannot_promote_catia_replay'] = TestStripped

# this binding exists because shared behavior needs one stable value
globals()['test_swapped_native_document_cannot_exact_replay'] = TestSwappedDoc

# this binding exists because shared behavior needs one stable value
globals()['test_unresolved_catpart_visibility_fails_closed_and_reverses_exactly'] = TestUnresolved
